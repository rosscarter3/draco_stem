...
def compute_topomesh_property(topomesh, property_name, degree=0, positions=None, normal_method="density",
                              object_positions=None, object_radius=10., verbose=False):
    ...
    if property_name == 'barycenter':
        ...
        if degree == 0:
            topomesh.update_wisp_property('barycenter', degree=degree, values=positions,
                                          keys=np.array(list(topomesh.wisps(degree))))
        else:
            ...
            topomesh.update_wisp_property('barycenter', degree=degree, values=barycenter_positions,
                                          keys=barycentrable_elements)
    ...
    if property_name == 'vertices':
        ...
        if degree == 0:
            topomesh.update_wisp_property('vertices', degree=degree,
                                          values=np.array(list(topomesh.wisps(degree))).astype(int),
                                          keys=np.array(list(topomesh.wisps(degree))))
        else:
            print list(topomesh.wisps(degree))
            topomesh.update_wisp_property('vertices', degree=degree, values=np.array(
                [np.unique(list(topomesh.borders(degree, w, degree))).astype(int) for w in topomesh.wisps(degree)]),
                                          keys=np.array(list(topomesh.wisps(degree))))
        if degree == 1:
            topomesh.update_wisp_property('borders', degree=degree, values=np.array(
                [np.unique(list(topomesh.borders(degree, w, degree))).astype(int) for w in topomesh.wisps(degree)]),
                                          keys=np.array(list(topomesh.wisps(degree))))
    ...
    if property_name == 'edges':
        ...
        if degree < 1:
            topomesh.update_wisp_property('edges', degree=degree, values=np.array(
                [np.array(list(topomesh.regions(degree, w, 1 - degree))).astype(int) for w in topomesh.wisps(degree)]),
                                          keys=np.array(list(topomesh.wisps(degree))))
        elif degree > 1:
            topomesh.update_wisp_property('edges', degree=degree, values=np.array(
                [np.array(list(topomesh.borders(degree, w, degree - 1))).astype(int) for w in topomesh.wisps(degree)]),
                                          keys=np.array(list(topomesh.wisps(degree))))
        else:
            topomesh.update_wisp_property('edges', degree=degree,
                                          values=np.array(list(topomesh.wisps(degree))).astype(int),
                                          keys=np.array(list(topomesh.wisps(degree))))
    ...
    if property_name == 'triangles':
        ...
        if degree < 2:
            topomesh.update_wisp_property('triangles', degree=degree, values=np.array(
                [np.array(list(topomesh.regions(degree, w, 2 - degree))).astype(int) for w in topomesh.wisps(degree)]),
                                          keys=np.array(list(topomesh.wisps(degree))))
        elif degree > 2:
            topomesh.update_wisp_property('triangles', degree=degree, values=np.array(
                [np.array(list(topomesh.borders(degree, w, degree - 2))).astype(int) for w in topomesh.wisps(degree)]),
                                          keys=np.array(list(topomesh.wisps(degree))))
        else:
            topomesh.update_wisp_property('triangles', degree=degree,
                                          values=np.array(list(topomesh.wisps(degree))).astype(int),
                                          keys=np.array(list(topomesh.wisps(degree))))
    ...
    if property_name == 'cells':
        ...
        if degree == topomesh.degree():
            topomesh.update_wisp_property('cells', degree=degree,
                                          values=np.array(list(topomesh.wisps(degree))).astype(int),
                                          keys=np.array(list(topomesh.wisps(degree))))
        else:
            topomesh.update_wisp_property('cells', degree=degree, values=np.array(
                [[int(c) for c in topomesh.regions(degree, w, topomesh.degree() - degree)] for w in
                 topomesh.wisps(degree)]), keys=np.array(list(topomesh.wisps(degree))))
    ...
    if property_name == 'borders':
        ...
        topomesh.update_wisp_property('borders', degree=degree, values=np.array(
            [np.array(list(topomesh.borders(degree, w)), int) for w in topomesh.wisps(degree)]),
                                      keys=np.array(list(topomesh.wisps(degree))))
    ...
    if property_name == 'border_neighbors':
        ...
        topomesh.update_wisp_property('neighbors', degree=degree, values=np.array(
            [np.unique(list(topomesh.border_neighbors(degree, w))).astype(int) for w in topomesh.wisps(degree)]),
                                      keys=np.array(list(topomesh.wisps(degree))))
    ...
    if property_name == 'regions':
        ...
        topomesh.update_wisp_property('regions', degree=degree, values=np.array(
            [np.array(list(topomesh.regions(degree, w))).astype(int) for w in topomesh.wisps(degree)]),
                                      keys=np.array(list(topomesh.wisps(degree))))
    ...
    if property_name == 'region_neighbors':
        ...
        if not 'neighbors' in topomesh.wisp_property_names(degree):
            topomesh.add_wisp_property('neighbors', degree=degree)
        topomesh.update_wisp_property('neighbors', degree=degree, values=np.array(
            [np.unique(list(topomesh.region_neighbors(degree, w))).astype(int) for w in topomesh.wisps(degree)]),
                                      keys=np.array(list(topomesh.wisps(degree))))
    ...
    if property_name == 'valence':
        ...
        # topomesh.update_wisp_property('valence',
        #                               degree=degree,
        #                               values=np.array([len(topomesh.wisp_property('neighbors',degree)[w])
        #                                                for w in topomesh.wisps(degree)]),
        #                               keys=np.array(list(topomesh.wisps(degree))))
        topomesh.update_wisp_property('valence', degree=degree, values=np.array(
            [len(list(topomesh.region_neighbors(degree, w))) for w in topomesh.wisps(degree)]).astype(int),
                                      keys=np.array(list(topomesh.wisps(degree))))
        ...
        if degree == 2:
            ...
            for t, fid in enumerate(topomesh.wisps(2)):
                ...
                while len(oriented_face_eids) < len(face_eids):
                    ...
                    candidate_eids = set(list(topomesh.regions(0, end_pid))).intersection(
                        set(list(face_eids))).difference({current_eid})
            ...
            topomesh.update_wisp_property('oriented_borders', degree=2, values=np.array(
                [(b, o) for b, o in zip(oriented_borders, oriented_border_orientations)]),
                                          keys=np.array(list(topomesh.wisps(2))))
            ...
            for f, fid in enumerate(topomesh.wisps(2)):
                ...
                if len(face_vertices) == 3:
                    ...
                    face_normal = np.cross(vertices_positions[1] - vertices_positions[0],
                    ...
                    vertices_positions = np.array([np.concatenate([p, [face_barycenter]]) for p in
                                                   topomesh.wisp_property('barycenter', 0).values(face_edges)])
                    face_triangle_normals = np.cross(vertices_positions[:, 1] - vertices_positions[:, 0],
                                                     vertices_positions[:, 2] - vertices_positions[:, 0])
                    face_triangle_normals = face_triangle_normals / np.linalg.norm(face_triangle_normals,
                                                                                   axis=1)[:, np.newaxis]
                    face_triangle_lengths = np.linalg.norm(
                        vertices_positions[..., triangle_edge_list, 1] - vertices_positions[..., triangle_edge_list, 0],
                        axis=2)
                    face_triangle_perimeters = face_triangle_lengths.sum(axis=1)
                    face_triangle_areas = np.sqrt((face_triangle_perimeters / 2.0) * (
                        face_triangle_perimeters / 2.0 - face_triangle_lengths[..., 0]) * (
                                                      face_triangle_perimeters / 2.0 - face_triangle_lengths[
                                                          ..., 1]) * (
                                                      face_triangle_perimeters / 2.0 - face_triangle_lengths[..., 2]))
                    face_normal = (face_triangle_normals * face_triangle_areas[:, np.newaxis]).sum(
                        axis=0) / face_triangle_areas.sum()
                face_normal /= np.linalg.norm(face_normal)
                oriented_face_normals += [face_normal]
            topomesh.update_wisp_property('normal', degree=2, values=np.array(oriented_face_normals),
                                          keys=np.array(list(topomesh.wisps(2))))
            ...
            for c, cid in enumerate(topomesh.wisps(3)):
                ...
                cell_fid_dot_product = np.einsum('ij,ij->i', cell_fid_normals,
                                                 cell_fid_barycenters - cell_barycenter[np.newaxis])
                ...
                for eid in topomesh.wisp_property('oriented_borders', 2)[start_fid][0]:
                    candidate_fids = set(list(topomesh.regions(1, eid))).intersection(set(list(cell_fids))).difference(
                        set(oriented_cell_fid_orientations.keys()))
                ...
                while len(oriented_cell_fid_orientations.keys()) < len(cell_fids):
                    ...
                    while len(next_fid_queue) > 0:
                        ...
                        current_face_eid_orientations = array_dict(
                            *topomesh.wisp_property('oriented_borders', 2)[current_fid][::-1])
                        current_fid_orientation = oriented_cell_fid_orientations[current_fid]
                        next_face_eid_orientations = array_dict(
                            *topomesh.wisp_property('oriented_borders', 2)[next_fid][::-1])
                        ...
                        next_fid_orientation = - current_face_eid_orientations[
                            transition_eid] * current_fid_orientation * next_face_eid_orientations[transition_eid]
                    ...
                    if len(oriented_cell_fid_orientations.keys()) < len(cell_fids):
                        remaining_cell_fids = np.array(
                            list(set(cell_fids).difference(set(oriented_cell_fid_orientations.keys()))))
                        start_fid = remaining_cell_fids[
                            np.argmax(np.abs(cell_fid_dot_product.values(remaining_cell_fids)))]
                        oriented_cell_fid_orientations[start_fid] = cell_fid_orientation[start_fid]
                        ...
                        eid_orientations = array_dict(*topomesh.wisp_property('oriented_borders', 2)[start_fid][::-1])
                        for eid in topomesh.wisp_property('oriented_borders', 2)[start_fid][0]:
                            candidate_fids = set(list(topomesh.regions(1, eid))).intersection(
                                set(list(cell_fids))).difference(set(oriented_cell_fid_orientations.keys()))
                            if len(candidate_fids) > 0:
                                for fid in candidate_fids:
                                    next_fid_queue.insert(0, fid)
                                    current_fid_queue.insert(0, start_fid)
                ...
                oriented_border_orientations += [
                    [oriented_cell_fid_orientations[b] for b in oriented_cell_fid_orientations.keys()]]
                oriented_border_components += [[fid_components[b] for b in oriented_cell_fid_orientations.keys()]]

            topomesh.update_wisp_property('oriented_borders', degree=3, values=np.array(
                [(b, o) for b, o in zip(oriented_borders, oriented_border_orientations)]),
                                          keys=np.array(list(topomesh.wisps(3))))
            topomesh.update_wisp_property('oriented_border_components', degree=3, values=np.array(
                [(b, c) for b, c in zip(oriented_borders, oriented_border_components)]),
                                          keys=np.array(list(topomesh.wisps(3))))
            # oriented_face_pids = [list(topomesh.borders(1,eid))[0 if ori==1 else 1]
            # for eid,ori in zip(oriented_face_eids,oriented_face_eid_orientations)]
    ...
    if property_name == 'oriented_vertices':
        if degree == 2:
            ...
            for t, fid in enumerate(topomesh.wisps(2)):
                ...
                oriented_face_pids = [list(topomesh.borders(1, eid))[0 if ori == 1 else 1] for eid, ori in
                                      zip(oriented_face_eids, oriented_face_eid_orientations)]
                oriented_vertices += [oriented_face_pids]
            topomesh.update_wisp_property('oriented_vertices', degree=2, values=np.array(oriented_vertices),
                                          keys=np.array(list(topomesh.wisps(2))))
    ...
    if property_name == 'length':
        ...
        vertices_positions = positions.values(
            topomesh.wisp_property('borders', degree).values(list(topomesh.wisps(degree))))
        edge_vectors = vertices_positions[:, 1] - vertices_positions[:, 0]
        edge_lengths = np.power(np.sum(np.power(edge_vectors, 2.0), axis=1), 0.5)
        topomesh.update_wisp_property('length', degree=degree, values=edge_lengths,
                                      keys=np.array(list(topomesh.wisps(degree))))
    ...
    if property_name == 'perimeter':
        ...
        if not topomesh.has_wisp_property('length', degree=1, is_computed=True):
            compute_topomesh_property(topomesh, 'length', degree=1)
        edge_lengths = topomesh.wisp_property('length', degree=1).values(
            topomesh.wisp_property('borders', degree=degree).values(list(topomesh.wisps(degree))))
        triangle_perimeters = np.sum(edge_lengths, axis=1)
        topomesh.update_wisp_property('perimeter', degree=degree, values=triangle_perimeters,
                                      keys=np.array(list(topomesh.wisps(degree))))
    ...
    if property_name == 'area':
        ...
        edge_lengths = topomesh.wisp_property('length', degree=1).values(
            topomesh.wisp_property('borders', degree=degree).values(list(topomesh.wisps(degree))))
        triangle_perimeters = np.sum(edge_lengths, axis=1)
        triangle_areas = np.sqrt((triangle_perimeters / 2.0) * (triangle_perimeters / 2.0 - edge_lengths[:, 0]) * (
            triangle_perimeters / 2.0 - edge_lengths[:, 1]) * (triangle_perimeters / 2.0 - edge_lengths[:, 2]))
        topomesh.update_wisp_property('area', degree=degree, values=triangle_areas,
                                      keys=np.array(list(topomesh.wisps(degree))))
    ...
    if property_name == 'eccentricity':
        ...
        edge_index_list = np.array([[1, 2], [0, 1], [0, 2]])
        triangle_edge_vertices = triangle_vertices[:, edge_index_list]
        triangle_edge_vectors = topomesh.wisp_property('barycenter', degree=0).values(
            triangle_edge_vertices[..., 1]) - topomesh.wisp_property('barycenter', degree=0).values(
            triangle_edge_vertices[..., 0])
        triangle_edge_lengths = np.linalg.norm(triangle_edge_vectors, axis=2)

        triangle_sinuses = np.zeros_like(triangle_edge_lengths, np.float32)
        triangle_sinuses[:, 0] = np.power(np.array(1.0 - np.power(
            (triangle_edge_lengths[:, 1] ** 2 + triangle_edge_lengths[:, 2] ** 2 - triangle_edge_lengths[:, 0] ** 2) / (
                2.0 * triangle_edge_lengths[:, 1] * triangle_edge_lengths[:, 2]), 2.0), np.float16), 0.5)
        triangle_sinuses[:, 1] = np.power(np.array(1.0 - np.power(
            (triangle_edge_lengths[:, 0] ** 2 + triangle_edge_lengths[:, 1] ** 2 - triangle_edge_lengths[:, 2] ** 2) / (
                2.0 * triangle_edge_lengths[:, 0] * triangle_edge_lengths[:, 1]), 2.0), np.float16), 0.5)
        triangle_sinuses[:, 2] = np.power(np.array(1.0 - np.power(
            (triangle_edge_lengths[:, 2] ** 2 + triangle_edge_lengths[:, 0] ** 2 - triangle_edge_lengths[:, 1] ** 2) / (
                2.0 * triangle_edge_lengths[:, 2] * triangle_edge_lengths[:, 0]), 2.0), np.float16), 0.5)

        triangle_sinus_eccentricities = 1.0 - (2.0 * (
            triangle_sinuses[:, 0] + triangle_sinuses[:, 1] + triangle_sinuses[:, 2])) / (3. * np.sqrt(3.))
        topomesh.update_wisp_property('eccentricity', degree=degree, values=triangle_sinus_eccentricities,
                                      keys=np.array(list(topomesh.wisps(degree))))
    ...
    if property_name == 'normal':
        if degree == 2:
            ...
            if normal_method == "orientation":
                ...
                triangle_orientations = array_dict(topomesh.wisp_property('oriented_borders', 3)[normal_cell][1],
                                                   topomesh.wisp_property('oriented_borders', 3)[normal_cell][0])
                topomesh.update_wisp_property('orientation', 2, triangle_orientations.values(list(topomesh.wisps(2))),
                                              list(topomesh.wisps(2)))
                ...
                compute_topomesh_property(topomesh, 'oriented_vertices', 2)
                vertices_positions = topomesh.wisp_property('barycenter', 0).values(
                    topomesh.wisp_property('oriented_vertices', 2).values())
                normal_vectors = np.cross(vertices_positions[:, 1] - vertices_positions[:, 0],
                                          vertices_positions[:, 2] - vertices_positions[:, 0])
                normal_norms = np.linalg.norm(normal_vectors, axis=1)
                normal_orientations = topomesh.wisp_property('orientation', 2).values()

                face_vectors = vertices_positions.mean(axis=1) - topomesh.wisp_property('barycenter', 0).values().mean(
                    axis=0)
                global_normal_orientation = np.sign(
                    np.einsum("...ij,...ij->...i", normal_orientations[:, np.newaxis] * normal_vectors,
                              face_vectors).mean())
                topomesh.update_wisp_property('normal', 2, global_normal_orientation * normal_orientations[:,
                                                                                       np.newaxis] * normal_vectors / normal_norms[
                                                                                                                      :,
                                                                                                                      np.newaxis],
                                              list(topomesh.wisps(2)))
                ...
                vertices_positions = positions.values(
                    topomesh.wisp_property('vertices', degree).values(list(topomesh.wisps(degree))))
                normal_vectors = np.cross(vertices_positions[:, 1] - vertices_positions[:, 0],
                                          vertices_positions[:, 2] - vertices_positions[:, 0])

                # reversed_normals = np.where(normal_vectors[:,2] < 0)[0]

                from implicit_surfaces import point_spherical_density
                ...
                triangle_exterior_density = point_spherical_density(object_positions,
                                                                    topomesh.wisp_property('barycenter', 2).values()[
                                                                        triangle_epidermis] + normal_vectors[
                                                                        triangle_epidermis],
                                                                    sphere_radius=object_radius, k=0.5)
                triangle_interior_density = point_spherical_density(object_positions,
                                                                    topomesh.wisp_property('barycenter', 2).values()[
                                                                        triangle_epidermis] - normal_vectors[
                                                                        triangle_epidermis],
                                                                    sphere_radius=object_radius, k=0.5)
                normal_orientation = 2 * (triangle_exterior_density < triangle_interior_density) - 1
                normal_vectors[triangle_epidermis] = normal_orientation[..., np.newaxis] * normal_vectors[
                    triangle_epidermis]
                normal_norms = np.linalg.norm(normal_vectors, axis=1)
                # normal_norms[np.where(normal_norms==0)] = 0.001
                topomesh.update_wisp_property('normal', degree=degree,
                                              values=normal_vectors / normal_norms[:, np.newaxis],
                                              keys=np.array(list(topomesh.wisps(degree))))
            ...
            epidermis_triangles = np.array(list(topomesh.wisps(2)))[
                topomesh.wisp_property('epidermis', 2).values(list(topomesh.wisps(2)))]
            # print epidermis_triangles

            # triangle_vertices = topomesh.wisp_property('vertices',degree=2).values(list(topomesh.wisps(2)))
            triangle_vertices = topomesh.wisp_property('vertices', degree=2).values(epidermis_triangles)
            rotated_triangle_vertices = np.transpose(
                [triangle_vertices[:, 2], triangle_vertices[:, 0], triangle_vertices[:, 1]])
            antirotated_triangle_vertices = np.transpose(
                [triangle_vertices[:, 1], triangle_vertices[:, 2], triangle_vertices[:, 0]])
            triangle_vertices = np.append(np.append(triangle_vertices, rotated_triangle_vertices, axis=0),
                                          antirotated_triangle_vertices, axis=0)

            # triangle_areas = np.tile(topomesh.wisp_property('area',degree=2).values(list(topomesh.wisps(2))),(3))
            # triangle_normal_vectors = np.tile(topomesh.wisp_property('normal',degree=2).values(list(topomesh.wisps(2))),(3,1))
            triangle_areas = np.tile(topomesh.wisp_property('area', degree=2).values(epidermis_triangles), 3)
            triangle_normal_vectors = np.tile(topomesh.wisp_property('normal', degree=2).values(epidermis_triangles),
                                              (3, 1))

            vertex_normal_vectors = np.transpose([nd.sum(triangle_normal_vectors[:, k] * triangle_areas,
                                                         triangle_vertices[:, 0],
                                                         index=np.array(list(topomesh.wisps(0)))) for k in [0, 1, 2]])
            normal_norms = np.linalg.norm(vertex_normal_vectors, axis=1)
            normal_norms[np.where(normal_norms == 0)] = 0.001
            topomesh.update_wisp_property('normal', degree=degree,
                                          values=vertex_normal_vectors / normal_norms[:, np.newaxis],
                                          keys=np.array(list(topomesh.wisps(degree))))
    ...
    if property_name == 'angles':
        ...
        triangle_cosines = np.zeros_like(triangle_edge_lengths, np.float32)
        triangle_cosines[:, 0] = (triangle_edge_lengths[:, 1] ** 2 + triangle_edge_lengths[:,
                                                                     2] ** 2 - triangle_edge_lengths[:, 0] ** 2) / (
                                     2.0 * triangle_edge_lengths[:, 1] * triangle_edge_lengths[:, 2])
        triangle_cosines[:, 1] = (triangle_edge_lengths[:, 0] ** 2 + triangle_edge_lengths[:,
                                                                     1] ** 2 - triangle_edge_lengths[:, 2] ** 2) / (
                                     2.0 * triangle_edge_lengths[:, 0] * triangle_edge_lengths[:, 1])
        triangle_cosines[:, 2] = (triangle_edge_lengths[:, 2] ** 2 + triangle_edge_lengths[:,
                                                                     0] ** 2 - triangle_edge_lengths[:, 1] ** 2) / (
                                     2.0 * triangle_edge_lengths[:, 2] * triangle_edge_lengths[:, 0])
        triangle_angles = np.arccos(triangle_cosines)
        topomesh.update_wisp_property('angles', degree=degree, values=triangle_angles,
                                      keys=np.array(list(topomesh.wisps(degree))))
    ...
    if property_name == 'incircle center':
        ...
        triangle_sinuses[:, 0] = np.sqrt(1.0 - np.power(
            (triangle_edge_lengths[:, 1] ** 2 + triangle_edge_lengths[:, 2] ** 2 - triangle_edge_lengths[:, 0] ** 2) / (
                2.0 * triangle_edge_lengths[:, 1] * triangle_edge_lengths[:, 2]), 2.0))
        triangle_sinuses[:, 1] = np.sqrt(1.0 - np.power(
            (triangle_edge_lengths[:, 0] ** 2 + triangle_edge_lengths[:, 1] ** 2 - triangle_edge_lengths[:, 2] ** 2) / (
                2.0 * triangle_edge_lengths[:, 0] * triangle_edge_lengths[:, 1]), 2.0))
        triangle_sinuses[:, 2] = np.sqrt(1.0 - np.power(
            (triangle_edge_lengths[:, 2] ** 2 + triangle_edge_lengths[:, 0] ** 2 - triangle_edge_lengths[:, 1] ** 2) / (
                2.0 * triangle_edge_lengths[:, 2] * triangle_edge_lengths[:, 0]), 2.0))

        # print triangle_sinuses.shape
        # print triangle_vertices.shape

        triangle_centers = ((
                                topomesh.wisp_property('barycenter', degree=0).values(triangle_vertices) /
                                triangle_sinuses[
                                    ..., np.newaxis]).sum(axis=1)) / (
                               (1. / triangle_sinuses).sum(axis=1)[..., np.newaxis])
        # print triangle_centers
        # print triangle_centers.shape
        topomesh.update_wisp_property('incircle center', degree=degree, values=triangle_centers,
                                      keys=np.array(list(topomesh.wisps(degree))))
    ...
    if property_name == 'volume':
        ...
        cell_triangles = np.array(
            np.concatenate(topomesh.wisp_property('borders', degree=3).values(list(topomesh.wisps(degree)))), int)
        cell_tetrahedra_cells = np.array(
            np.concatenate([[c for t in topomesh.wisp_property('borders', degree=3)[c]] for c in topomesh.wisps(3)]),
            int)
        cell_triangle_vertices = topomesh.wisp_property('vertices', degree=2).values(cell_triangles)
        cell_tetrahedra = np.concatenate(
            [topomesh.wisp_property('barycenter', degree=3).values(cell_tetrahedra_cells)[:, np.newaxis],
             topomesh.wisp_property('barycenter', degree=0).values(cell_triangle_vertices)], axis=1)

        cell_tetra_matrix = np.transpose(
            np.array([cell_tetrahedra[:, 1], cell_tetrahedra[:, 2], cell_tetrahedra[:, 3]]) - cell_tetrahedra[:, 0],
            axes=(1, 2, 0))
        ...
        topomesh.update_wisp_property('volume', degree=degree, values=cell_volumes,
                                      keys=np.array(list(topomesh.wisps(degree))))
    ...
    if property_name == 'surface':
        ...
        topomesh.update_wisp_property('surface', degree=degree, values=cell_surfaces,
                                      keys=np.array(list(topomesh.wisps(degree))))
    ...
    if property_name == 'convexhull_surface':
        ...
        for c in topomesh.wisps(3):
            ...
            if len(cell_vertices) > 0:
                convex_hull = pgl.Fit(
                    pgl.Point3Array(topomesh.wisp_property('barycenter', 0).values(cell_vertices))).convexHull()
    ...
    if property_name == 'convexity':
        ...
        topomesh.update_wisp_property('convexity', degree=degree, values=topomesh.wisp_property('volume', 3).values(
            topomesh.wisp_property('convexhull_volume', 3).keys()) / topomesh.wisp_property('convexhull_volume',
                                                                                            3).values(
            topomesh.wisp_property('convexhull_volume', 3).keys()),
                                      keys=topomesh.wisp_property('convexhull_volume', 3).keys())
    ...
    if property_name == 'epidermis':
        ...
        if degree != 2:
            ...
            if not topomesh.has_wisp_property('epidermis', degree=2, is_computed=True):
                compute_topomesh_property(topomesh, 'epidermis', degree=2)
            epidermis = np.array([topomesh.wisp_property('epidermis', degree=2).values(
                topomesh.wisp_property('triangles', degree=degree)[w]).any() for w in topomesh.wisps(degree)])
            topomesh.update_wisp_property('epidermis', degree=degree, values=epidermis,
                                          keys=np.array(list(topomesh.wisps(degree))))
        else:
            if not topomesh.has_wisp_property('cells', degree=2, is_computed=True):
                compute_topomesh_property(topomesh, 'cells', degree=2)
            topomesh.update_wisp_property('epidermis', degree=2, values=(
                np.array(map(len, topomesh.wisp_property('cells', 2).values(list(topomesh.wisps(2))))) == 1),
                                          keys=np.array(list(topomesh.wisps(2))))
    ...
    if property_name in ['principal_curvatures', 'principal_curvature_tensor', 'principal_direction_min',
                         'principal_direction_max', 'principal_curvature_min', 'principal_curvature_max',
        ...
        if degree == 0:
            ...
            epidermis_vertices = np.array(list(topomesh.wisps(0)))[
                topomesh.wisp_property('epidermis', 0).values(list(topomesh.wisps(0)))]

            vertex_neighbor_vertices = np.concatenate(
                [[n for n in topomesh.wisp_property('neighbors', 0)[v] if n in epidermis_vertices] for v in
                 epidermis_vertices])
            vertex_neighbor_vertex = np.concatenate(
                [[v for n in topomesh.wisp_property('neighbors', 0)[v] if n in epidermis_vertices] for v in
                 epidermis_vertices])

            vertex_neighbor_vectors = topomesh.wisp_property('barycenter', 0).values(
                vertex_neighbor_vertex) - topomesh.wisp_property('barycenter', 0).values(vertex_neighbor_vertices)

            vertex_neighbor_projected_vectors = vertex_neighbor_vectors - np.einsum('ij,ij->i', vertex_neighbor_vectors,
                                                                                    vertex_normal_vectors.values(
                                                                                        vertex_neighbor_vertex))[:,
                                                                          np.newaxis] * vertex_normal_vectors.values(
                vertex_neighbor_vertex)
            vertex_neighbor_projected_vectors = vertex_neighbor_projected_vectors / np.linalg.norm(
                vertex_neighbor_projected_vectors, axis=1)[:, np.newaxis]

            vertex_neighbor_directional_curvature = 2. * np.einsum('ij,ij->i',
                                                                   vertex_normal_vectors.values(vertex_neighbor_vertex),
                                                                   vertex_neighbor_vectors) / np.power(
                np.linalg.norm(vertex_neighbor_vectors, axis=1), 2.0)

            # epidermis_triangles = np.array(list(topomesh.wisps(2)))[topomesh.wisp_property('epidermis',2).values(list(topomesh.wisps(2)))]
            compute_topomesh_property(topomesh, 'triangles', 0)
            vertex_neighbor_triangles = np.array([[t for t in topomesh.wisp_property('triangles', 0)[v] if
                                                   (t in topomesh.wisp_property('triangles', 0)[n]) and (
                                                       topomesh.wisp_property('epidermis', 2)[t])] for (v, n) in
                                                  zip(vertex_neighbor_vertex, vertex_neighbor_vertices)])
            ...
            vertex_neighbor_projected_matrix = np.einsum('...ji,...ij->...ij',
                                                         vertex_neighbor_projected_vectors[..., np.newaxis],
                                                         vertex_neighbor_projected_vectors[..., np.newaxis])
            vertex_neighbor_curvature_matrix = vertex_neighbor_triangle_area[:, np.newaxis,
                                               np.newaxis] * vertex_neighbor_directional_curvature[:, np.newaxis,
                                                             np.newaxis] * vertex_neighbor_projected_matrix

            vertex_curvature_matrix = np.zeros((epidermis_vertices.shape[0], 3, 3))
            for i in xrange(3):
                for j in xrange(3):
                    vertex_curvature_matrix[:, i, j] = nd.sum(vertex_neighbor_curvature_matrix[:, i, j],
                                                              vertex_neighbor_vertex, index=epidermis_vertices)
            vertex_curvature_matrix = vertex_curvature_matrix / (
                nd.sum(vertex_neighbor_triangle_area, vertex_neighbor_vertex, index=epidermis_vertices)[:, np.newaxis,
                np.newaxis] + np.power(10., -10))

            vertex_curvature_tensor = array_dict(vertex_curvature_matrix, epidermis_vertices)

            try:
                vertex_curvature_matrix_eigenvalues, vertex_curvature_matrix_eigenvectors = np.linalg.eig(
                    vertex_curvature_matrix)
                ...
                vertex_curvature_matrix_eigenvalues_trunc, vertex_curvature_matrix_eigenvectors_trunc = np.linalg.eig(
                    vertex_curvature_matrix_trunc)
            ...
            vertex_principal_curvature_min = array_dict(
                [np.sort(v[np.argsort(np.abs(v))[1:]])[0] for v in vertex_curvature_matrix_eigenvalues],
                epidermis_vertices)
            vertex_principal_curvature_max = array_dict(
                [np.sort(v[np.argsort(np.abs(v))[1:]])[1] for v in vertex_curvature_matrix_eigenvalues],
                epidermis_vertices)
            vertex_principal_direction_min = array_dict([vec[np.argsort(v[np.argsort(np.abs(v))[1:]])[0]] for vec, v in
                                                         zip(vertex_curvature_matrix_eigenvectors,
                                                             vertex_curvature_matrix_eigenvalues)], epidermis_vertices)
            vertex_principal_direction_max = array_dict([vec[np.argsort(v[np.argsort(np.abs(v))[1:]])[1]] for vec, v in
                                                         zip(vertex_curvature_matrix_eigenvectors,
                                                             vertex_curvature_matrix_eigenvalues)], epidermis_vertices)

            topomesh.update_wisp_property('principal_curvature_tensor', 0, np.array(
                [vertex_curvature_tensor[v] if v in epidermis_vertices else np.zeros((3, 3), float) for v in
                 topomesh.wisps(0)]), np.array(list(topomesh.wisps(0))))

            topomesh.update_wisp_property('principal_direction_min', 0, np.array(
                [vertex_principal_direction_min[v] if v in epidermis_vertices else np.array([0., 0., 0.]) for v in
                 topomesh.wisps(0)]), np.array(list(topomesh.wisps(0))))
            topomesh.update_wisp_property('principal_direction_max', 0, np.array(
                [vertex_principal_direction_max[v] if v in epidermis_vertices else np.array([0., 0., 0.]) for v in
                 topomesh.wisps(0)]), np.array(list(topomesh.wisps(0))))

            topomesh.update_wisp_property('principal_curvature_min', 0, np.array(
                [vertex_principal_curvature_min[v] if v in epidermis_vertices else 0. for v in topomesh.wisps(0)]),
                                          np.array(list(topomesh.wisps(0))))
            topomesh.update_wisp_property('principal_curvature_max', 0, np.array(
                [vertex_principal_curvature_max[v] if v in epidermis_vertices else 0. for v in topomesh.wisps(0)]),
                                          np.array(list(topomesh.wisps(0))))
            topomesh.update_wisp_property('mean_curvature', 0, (
                topomesh.wisp_property('principal_curvature_max', 0).values(
                    list(topomesh.wisps(0))) + topomesh.wisp_property('principal_curvature_min', 0).values(
                    list(topomesh.wisps(0)))) / 2., np.array(list(topomesh.wisps(0))))
            topomesh.update_wisp_property('gaussian_curvature', 0,
                                          topomesh.wisp_property('principal_curvature_max', 0).values(
                                              list(topomesh.wisps(0))) * topomesh.wisp_property(
                                              'principal_curvature_min', 0).values(list(topomesh.wisps(0))),
                                          np.array(list(topomesh.wisps(0))))
            ...
            epidermis_triangles = np.array(list(topomesh.wisps(2)))[
                topomesh.wisp_property('epidermis', 2).values(list(topomesh.wisps(2))).astype(bool)]
            triangle_vertices = topomesh.wisp_property('vertices', degree=2).values(epidermis_triangles)
            ...
            triangle_barycenter_normal_derivatives = triangle_barycenter_normal_derivatives[:, :,
                                                     1] - triangle_barycenter_normal_derivatives[:, :, 0]
            triangle_barycenter_normal_derivatives = triangle_barycenter_normal_derivatives / np.linalg.norm(
                triangle_barycenter_normals, axis=1)[:, np.newaxis, np.newaxis]

            triangle_barycenter_derivatives_projectors = np.transpose([np.einsum("...ij,...ij->...i",
                                                                                 triangle_barycenter_normals,
                                                                                 triangle_edge_vectors[:, k])[:,
                                                                       np.newaxis] * triangle_barycenter_normals for k
                                                                       in xrange(3)], (1, 0, 2))
            triangle_projected_barycenter_derivatives = triangle_edge_vectors - triangle_barycenter_derivatives_projectors

            E = np.einsum("...ij,...ij->...i", triangle_projected_barycenter_derivatives[:, 1],
                          triangle_projected_barycenter_derivatives[:, 1])
            F = np.einsum("...ij,...ij->...i", triangle_projected_barycenter_derivatives[:, 1],
                          triangle_projected_barycenter_derivatives[:, 2])
            G = np.einsum("...ij,...ij->...i", triangle_projected_barycenter_derivatives[:, 2],
                          triangle_projected_barycenter_derivatives[:, 2])

            L = -np.einsum("...ij,...ij->...i", triangle_barycenter_normal_derivatives[:, 1],
                           triangle_projected_barycenter_derivatives[:, 1])
            M1 = -np.einsum("...ij,...ij->...i", triangle_barycenter_normal_derivatives[:, 1],
                            triangle_projected_barycenter_derivatives[:, 2])
            M2 = -np.einsum("...ij,...ij->...i", triangle_barycenter_normal_derivatives[:, 2],
                            triangle_projected_barycenter_derivatives[:, 1])
            N = -np.einsum("...ij,...ij->...i", triangle_barycenter_normal_derivatives[:, 2],
                           triangle_projected_barycenter_derivatives[:, 2])
            ...
            weingarten_curvature_matrix_eigenvalues, weingarten_curvature_matrix_eigenvectors = np.linalg.eig(
                weingarten_curvature_matrix)
            ...
            weingarten_principal_curvature_min = weingarten_curvature_matrix_eigenvalues[tuple(
                [np.arange(len(epidermis_triangles)),
                 np.argsort(np.abs(weingarten_curvature_matrix_eigenvalues))[:, 0]])].astype(float)
            weingarten_principal_curvature_max = weingarten_curvature_matrix_eigenvalues[tuple(
                [np.arange(len(epidermis_triangles)),
                 np.argsort(np.abs(weingarten_curvature_matrix_eigenvalues))[:, 1]])].astype(float)
            weingarten_principal_vector_min = weingarten_curvature_matrix_eigenvectors[tuple(
                [np.arange(len(epidermis_triangles)),
                 np.argsort(np.abs(weingarten_curvature_matrix_eigenvalues))[:, 0]])].astype(float)
            weingarten_principal_vector_max = weingarten_curvature_matrix_eigenvectors[tuple(
                [np.arange(len(epidermis_triangles)),
                 np.argsort(np.abs(weingarten_curvature_matrix_eigenvalues))[:, 1]])].astype(float)
            ...
            weingarten_principal_direction_min = array_dict((weingarten_principal_vector_min[:, :,
                                                             np.newaxis] * triangle_projected_barycenter_derivatives[:,
                                                                           1:3]).sum(axis=1), epidermis_triangles)
            weingarten_principal_direction_max = array_dict((weingarten_principal_vector_max[:, :,
                                                             np.newaxis] * triangle_projected_barycenter_derivatives[:,
                                                                           1:3]).sum(axis=1), epidermis_triangles)

            P = np.transpose([weingarten_principal_direction_max.values(), weingarten_principal_direction_min.values(),
                              triangle_barycenter_normals], (1, 2, 0))
            D = np.array([np.diag(d) for d in np.transpose(
                [weingarten_principal_curvature_max, weingarten_principal_curvature_min,
                 np.zeros_like(epidermis_triangles)])])
            ...
            face_principal_curvature_min = array_dict(
                np.array([np.sort(v[np.argsort(np.abs(v))[1:]])[0] for v in face_curvature_matrix_eigenvalues]).astype(
                    float), epidermis_triangles)
            face_principal_curvature_max = array_dict(
                np.array([np.sort(v[np.argsort(np.abs(v))[1:]])[1] for v in face_curvature_matrix_eigenvalues]).astype(
                    float), epidermis_triangles)
            ...
            face_principal_direction_min = array_dict(np.array(
                [vec[np.argsort(v[np.argsort(np.abs(v))[1:]])[0]] for vec, v in
                 zip(face_curvature_matrix_eigenvectors, face_curvature_matrix_eigenvalues)]).astype(float),
                                                      epidermis_triangles)
            face_principal_direction_max = array_dict(np.array(
                [vec[np.argsort(v[np.argsort(np.abs(v))[1:]])[1]] for vec, v in
                 zip(face_curvature_matrix_eigenvectors, face_curvature_matrix_eigenvalues)]).astype(float),
                                                      epidermis_triangles)

            face_principal_curvature_tensor = array_dict(face_curvature_tensor, epidermis_triangles)
            topomesh.update_wisp_property('principal_curvature_tensor', 2, np.array(
                [face_principal_curvature_tensor[t] if t in epidermis_triangles else np.zeros((3, 3)) for t in
                 topomesh.wisps(2)]), np.array(list(topomesh.wisps(2))))

            topomesh.update_wisp_property('principal_direction_min', 2, np.array(
                [face_principal_direction_min[t] if t in epidermis_triangles else np.zeros(3) for t in
                 topomesh.wisps(2)]), np.array(list(topomesh.wisps(2))))
            topomesh.update_wisp_property('principal_direction_max', 2, np.array(
                [face_principal_direction_max[t] if t in epidermis_triangles else np.zeros(3) for t in
                 topomesh.wisps(2)]), np.array(list(topomesh.wisps(2))))

            topomesh.update_wisp_property('principal_curvature_min', 2, np.array(
                [face_principal_curvature_min[t] if t in epidermis_triangles else 0. for t in topomesh.wisps(2)]),
                                          np.array(list(topomesh.wisps(2))))
            topomesh.update_wisp_property('principal_curvature_max', 2, np.array(
                [face_principal_curvature_max[t] if t in epidermis_triangles else 0. for t in topomesh.wisps(2)]),
                                          np.array(list(topomesh.wisps(2))))
            topomesh.update_wisp_property('mean_curvature', 2, np.array([(face_principal_curvature_min[t] +
                                                                          face_principal_curvature_max[
                                                                              t]) / 2. if t in epidermis_triangles else 0.
                                                                         for t in topomesh.wisps(2)]),
                                          np.array(list(topomesh.wisps(2))))
            topomesh.update_wisp_property('gaussian_curvature', 2, np.array(
                [(face_principal_curvature_min[t] * face_principal_curvature_max[t]) if t in epidermis_triangles else 0.
                 for t in topomesh.wisps(2)]), np.array(list(topomesh.wisps(2))))
            ...
            epidermis_cells = np.array(list(topomesh.wisps(3)))[
                topomesh.wisp_property('epidermis', 3).values(list(topomesh.wisps(3)))]
            epidermis_vertices = np.array(list(topomesh.wisps(0)))[
                topomesh.wisp_property('epidermis', 0).values(list(topomesh.wisps(0)))]
            cell_epidermis_vertices = np.array(
                [np.intersect1d(topomesh.wisp_property('vertices', degree=3)[c], epidermis_vertices) for c in
                 epidermis_cells])

            def row_mean(array):
                return np.mean(array, axis=0)

            cell_curvature_tensor = array_dict(map(row_mean, topomesh.wisp_property('principal_curvature_tensor',
                                                                                    degree=0).values(
                cell_epidermis_vertices)), epidermis_cells)
            topomesh.update_wisp_property('principal_curvature_tensor', 3, np.array(
                [cell_curvature_tensor[c] if c in epidermis_cells else np.zeros((3, 3), float) for c in
                 topomesh.wisps(3)]), np.array(list(topomesh.wisps(3))))
            for direction in ['principal_direction_min', 'principal_direction_max']:
                cell_direction = array_dict(
                    map(row_mean, topomesh.wisp_property(direction, degree=0).values(cell_epidermis_vertices)),
                    epidermis_cells)
                topomesh.update_wisp_property(direction, 3, np.array(
                    [cell_direction[c] if c in epidermis_cells else np.array([0., 0., 0.]) for c in topomesh.wisps(3)]),
                                              np.array(list(topomesh.wisps(3))))
            for curvature in ['principal_curvature_min', 'principal_curvature_max', 'mean_curvature',
                              'gaussian_curvature']:
                cell_curvature = array_dict(
                    map(np.mean, topomesh.wisp_property(curvature, degree=0).values(cell_epidermis_vertices)),
                    epidermis_cells)
                topomesh.update_wisp_property(curvature, 3, np.array(
                    [cell_curvature[c] if c in epidermis_cells else 0. for c in topomesh.wisps(3)]),
                                              np.array(list(topomesh.wisps(3))))
    ...
    if property_name == 'cell_interface':
        ...
        if not topomesh.has_wisp_property('cells', degree=2, is_computed=True):
            compute_topomesh_property(topomesh, 'cells', degree=2)
        interface_triangles = np.array(list(topomesh.wisps(2)))[
            np.where(np.array(map(len, topomesh.wisp_property('cells', 2).values())) == 2)[0]]
    ...
    if property_name == 'interface':
        ...
        topomesh.update_interface_property('interface', degree=degree, values=np.array(
            map(np.intersect1d, neighbour_borders[:, 0], neighbour_borders[:, 1])),
                                           keys=np.array(list(topomesh.interfaces(degree))))
    ...
    if property_name == 'distance':
        ...
        topomesh.update_interface_property('interface', degree=degree, values=np.linalg.norm(neighbour_vectors, axis=1),
                                           keys=np.array(list(topomesh.interfaces(degree))))
...
def compute_topomesh_triangle_properties(topomesh, positions=None):
    ...
    edge_lengths = np.sort(
        topomesh.wisp_property('length', degree=1).values(topomesh.wisp_property('borders', degree=2).values()))

    triangle_perimeters = np.sum(edge_lengths, axis=1)
    if not 'perimeter' in topomesh.wisp_property_names(2):
        topomesh.add_wisp_property('perimeter', degree=2)
    topomesh.update_wisp_property('perimeter', degree=2, values=triangle_perimeters,
                                  keys=np.array(list(topomesh.wisps(2))))
    ...
    triangle_sinuses[:, 0] = np.sqrt(np.array(
        1.0 - np.power(edge_lengths[:, 1] ** 2 + edge_lengths[:, 2] ** 2 - edge_lengths[:, 0] ** 2, 2.0) / np.power(
            2.0 * edge_lengths[:, 1] * edge_lengths[:, 2], 2.0), np.float16))
    triangle_sinuses[:, 1] = np.sqrt(np.array(
        1.0 - np.power(edge_lengths[:, 2] ** 2 + edge_lengths[:, 0] ** 2 - edge_lengths[:, 1] ** 2, 2.0) / np.power(
            2.0 * edge_lengths[:, 2] * edge_lengths[:, 0], 2.0), np.float16))
    triangle_sinuses[:, 2] = np.sqrt(np.array(
        1.0 - np.power(edge_lengths[:, 0] ** 2 + edge_lengths[:, 1] ** 2 - edge_lengths[:, 2] ** 2, 2.0) / np.power(
            2.0 * edge_lengths[:, 0] * edge_lengths[:, 1], 2.0), np.float16))
    ...
    topomesh.update_wisp_property('eccentricity', degree=2, values=triangle_sinus_eccentricities,
                                  keys=np.array(list(topomesh.wisps(2))))
...
def compute_topomesh_vertex_property_from_faces(topomesh, property_name, weighting='area', neighborhood=1,
                                                adjacency_sigma=0.5):
    ...
    if neighborhood > 1:
        try:
            ...
            raise AssertionError(
                "\"" + weighting + "\" weighting can not be used with faces further than 1-ring (neighborhood > 1)")
    ...
    if neighborhood == 1:
        ...
        vertex_face_vertices = np.concatenate(
            [v * np.ones_like(list(topomesh.regions(0, v, 2))) for v in topomesh.wisps(0)]).astype(np.uint32)
    else:
        vertex_face_adjacency_matrix = topomesh.nb_wisps(2) * np.ones(
            (max(topomesh.wisps(0)) + 1, max(topomesh.wisps(2)) + 1))
        ...
        for dist in (np.arange(neighborhood)):
            ...
            neighbor_face_neighbor_vertex = np.concatenate(
                [v * np.ones_like(topomesh.wisp_property('neighbors', 2)[f]) for f, v in
                 zip(neighbor_faces, neighbor_face_vertices)])
            neighbor_face_neighbor_predecessor = np.concatenate(
                [f * np.ones_like(topomesh.wisp_property('neighbors', 2)[f]) for f in neighbor_faces])

            current_adjacency = vertex_face_adjacency_matrix[(neighbor_face_neighbor_vertex, neighbor_face_neighbors)]
            neighbor_adjacency = vertex_face_adjacency_matrix[
                                     (neighbor_face_neighbor_vertex, neighbor_face_neighbor_predecessor)] + 1
            ...
            vertex_face_adjacency_matrix[(neighbor_face_neighbor_vertex, neighbor_face_neighbors)] = np.minimum(
                current_adjacency, neighbor_adjacency)

        vertex_face_vertices, vertex_faces = np.where(vertex_face_adjacency_matrix < neighborhood)
        vertex_adjacency_gaussian_weight = \
            np.exp(-np.power(vertex_face_adjacency_matrix, 2.0) / (2 * np.power(adjacency_sigma, 2.)))[
                (vertex_face_vertices, vertex_faces)]
    ...
    if weighting == 'uniform':
        ...
        vertex_face_vertex_angles = np.array([angles[vertices == v] for v, vertices, angles in
                                              zip(vertex_face_vertices, vertex_face_face_vertices,
                                                  vertex_face_face_angles)])
        ...
        vertex_face_opposite_angles = np.array([angles[vertices != v] for v, vertices, angles in
                                                zip(vertex_face_vertices, vertex_face_face_vertices,
                                                    vertex_face_face_angles)])
        ...
        face_vertex_barycenter_radii = np.linalg.norm(face_vertex_positions - vertex_face_barycenter[:, np.newaxis],
                                                      axis=1)
    ...
    if vertex_face_property.ndim == 1:
        vertex_property = nd.sum(vertex_face_weight * vertex_face_property, vertex_face_vertices,
                                 index=list(topomesh.wisps(0))) / nd.sum(vertex_face_weight, vertex_face_vertices,
                                                                         index=list(topomesh.wisps(0)))
    elif vertex_face_property.ndim == 2:
        vertex_property = np.transpose([nd.sum(vertex_face_weight * vertex_face_property[:, k], vertex_face_vertices,
                                               index=list(topomesh.wisps(0))) / nd.sum(vertex_face_weight,
                                                                                       vertex_face_vertices,
                                                                                       index=list(topomesh.wisps(0)))
                                        for k in xrange(vertex_face_property.shape[1])])
    elif vertex_face_property.ndim == 3:
        vertex_property = np.transpose([[nd.sum(vertex_face_weight * vertex_face_property[:, j, k],
                                                vertex_face_vertices, index=list(topomesh.wisps(0))) / nd.sum(
            vertex_face_weight, vertex_face_vertices, index=list(topomesh.wisps(0))) for k in
                                         xrange(vertex_face_property.shape[2])] for j in
                                        xrange(vertex_face_property.shape[1])])
    ...
    topomesh.update_wisp_property(property_name, degree=0,
                                  values=array_dict(vertex_property, keys=list(topomesh.wisps(0))))
...
def compute_topomesh_cell_property_from_faces(topomesh, property_name, aggregate='mean', weighting='area'):
    ...
    cell_face_cells = np.concatenate([c * np.ones(topomesh.nb_borders(3, c)) for c in topomesh.wisps(3)]).astype(
        np.uint32)
    ...
    if cell_face_property.ndim == 1:
        cell_property = nd.sum(cell_face_weight * cell_face_property, cell_face_cells,
                               index=list(topomesh.wisps(3))) / nd.sum(cell_face_weight, cell_face_cells,
                                                                       index=list(topomesh.wisps(3)))
    elif cell_face_property.ndim == 2:
        cell_property = np.transpose([nd.sum(cell_face_weight * cell_face_property[:, k], cell_face_cells,
                                             index=list(topomesh.wisps(3))) / nd.sum(cell_face_weight, cell_face_cells,
                                                                                     index=list(topomesh.wisps(3))) for
                                      k in xrange(cell_face_property.shape[1])])
    elif cell_face_property.ndim == 3:
        cell_property = np.transpose([[nd.sum(cell_face_weight * cell_face_property[:, j, k], cell_face_cells,
                                              index=list(topomesh.wisps(3))) / nd.sum(cell_face_weight, cell_face_cells,
                                                                                      index=list(topomesh.wisps(3))) for
                                       k in xrange(cell_face_property.shape[2])] for j in
                                      xrange(cell_face_property.shape[1])])

    topomesh.update_wisp_property(property_name, degree=3,
                                  values=array_dict(cell_property, keys=list(topomesh.wisps(3))))
...
def filter_topomesh_property(topomesh, property_name, degree, coef=0.5, normalize=False):
    ...
    if degree > 1:
        neighbors = np.array([[(w1, w2) for w2 in topomesh.border_neighbors(degree, w1) if
                               not np.isnan(topomesh.wisp_property(property_name, degree=degree)[w2])] for w1 in
                              topomesh.wisps(degree) if
                              not np.isnan(topomesh.wisp_property(property_name, degree=degree)[w1])])
        ...
        neighbors = np.array([[(w1, w2) for w2 in topomesh.region_neighbors(degree, w1) if
                               np.isnan(topomesh.wisp_property(property_name, degree=degree)[w2]).sum() == 0] for w1 in
                              topomesh.wisps(degree) if
                              np.isnan(topomesh.wisp_property(property_name, degree=degree)[w1]).sum() == 0])
        neighbors = np.concatenate(neighbors[np.where(np.array(map(len, neighbors)) > 0)])
        # neighbors = topomesh.wisp_property('borders',degree-1).values()

    wisp_neighbor_degree = nd.sum(np.ones_like(neighbors[:, 0]), neighbors[:, 0],
                                  index=np.array(list(topomesh.wisps(degree))))

    neighbor_properties = topomesh.wisp_property(property_name, degree=degree).values(neighbors[:, 1])
    if neighbor_properties.ndim == 1:
        wisp_neighbor_property = nd.sum(neighbor_properties, neighbors[:, 0],
                                        index=np.array(list(topomesh.wisps(degree))))
        wisp_neighbor_property[np.where(wisp_neighbor_degree > 0)] = wisp_neighbor_property[
                                                                         np.where(wisp_neighbor_degree > 0)] / \
                                                                     wisp_neighbor_degree[
                                                                         np.where(wisp_neighbor_degree > 0)]
    else:
        wisp_neighbor_property = np.transpose(
            [nd.sum(neighbor_properties[:, d], neighbors[:, 0], index=np.array(list(topomesh.wisps(degree)))) for d in
             xrange(neighbor_properties.shape[1])])
        wisp_neighbor_property[np.where(wisp_neighbor_degree > 0)] = wisp_neighbor_property[
                                                                         np.where(wisp_neighbor_degree > 0)] / \
                                                                     wisp_neighbor_degree[
                                                                         np.where(wisp_neighbor_degree > 0)][:,
    ...
    filtered_property = (1. - coef) * topomesh.wisp_property(property_name, degree=degree).values(
        np.array(list(topomesh.wisps(degree)))) + coef * wisp_neighbor_property
    ...
    topomesh.update_wisp_property(property_name, degree=degree, values=filtered_property,
                                  keys=np.array(list(topomesh.wisps(degree))))


def topomesh_property_gaussian_filtering(topomesh, property_name, degree, neighborhood=3, adjacency_sigma=1.0,
                                         distance_sigma=1.0):
    """Filter an existing property by a gaussian-like operator.
    ...
    for i, wid in enumerate(topomesh.wisps(degree)):
        if degree > 1:
            adjacency_matrix[tuple([i * np.ones(topomesh.nb_border_neighbors(degree, wid), int),
                                    wisp_index.values(list(topomesh.border_neighbors(degree, wid)))])] = 1
        else:
            adjacency_matrix[tuple([i * np.ones(topomesh.nb_region_neighbors(degree, wid), int),
                                    wisp_index.values(list(topomesh.region_neighbors(degree, wid)))])] = 1
    ...
    for dist in (np.arange(neighborhood) + 1):
        for i, wid in enumerate(topomesh.wisps(degree)):
            ...
            neighbor_adjacency = adjacency_matrix[i][neighbor_neighbor_predecessor] + adjacency_matrix[neighbors][
                neighbor_neighbors]
    ...
    distance_matrix = np.array([vq(topomesh.wisp_property('barycenter', degree).values(),
                                   np.array([topomesh.wisp_property('barycenter', degree)[v]]))[1] for v in
                                topomesh.wisps(degree)])
    ...
    if properties.ndim == 1:
        ...
        filtered_properties = (vertex_gaussian[..., np.newaxis] * properties[:, np.newaxis]).sum(axis=0) / \
                              vertex_gaussian.sum(axis=0)[..., np.newaxis]

    topomesh.update_wisp_property(property_name, degree=degree, values=filtered_properties,
                                  keys=np.array(list(topomesh.wisps(degree))))
