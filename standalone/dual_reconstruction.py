...
def tetrahedra_dual_topomesh(image_tetrahedra, positions, voronoi=True, exterior=True, **kwargs):
    ...
    image_edge_lengths = np.linalg.norm(positions.values(image_edges[..., 1]) - positions.values(image_edges[..., 0]),
                                        axis=1)
    ...
    if kwargs.has_key('anisotropic_voronoi_tensors'):
        ...
        anisotropy_invert_tensors = array_dict(
            np.array([np.linalg.inv(anisotropy_tensors[c]) for c in anisotropy_tensors.keys()]),
            anisotropy_tensors.keys())
        ...
        if 1 in t:
            if exterior:
                ...
                for tetra_p in xrange(4):
                    triangle_matching.append(np.array(list(
                        vq(np.concatenate([image_tetrahedra[:, :tetra_p], image_tetrahedra[:, tetra_p + 1:]], axis=1),
                           np.array([vertex_triangle])))))
                ...
                vertex_triangle_normal = np.cross(positions[t[2]] - positions[t[1]],
                                                  positions[t[3]] - positions[t[1]]) / (
                                         np.linalg.norm(positions[t[2]] - positions[t[1]]) * np.linalg.norm(
                                             positions[t[3]] - positions[t[1]]))
                if len(vertex_other_tetra) > 0:
                    vertex_triangle_normal = np.sign(np.dot(vertex_triangle_normal,
                                                            vertex_triangle_barycenter - positions[
                                                                vertex_other_tetra_point])) * vertex_triangle_normal
                else:
                    vertex_triangle_normal = np.sign(np.dot(vertex_triangle_normal,
                                                            vertex_triangle_barycenter - tetrahedra_center)) * vertex_triangle_normal
                ...
                positions[1] = vertex_triangle_barycenter + neighbor_distance * np.sqrt(
                    3. / 2.) * vertex_triangle_normal
                ...
                if voronoi:
                    ...
                    # if False:
                    if anisotropic:
                        tetra_triangle_edge_vectors = positions.values(tetra_triangle_edges[..., 1]) - positions.values(
                            tetra_triangle_edges[..., 0])
                        tetra_triangle_edge_lengths = np.sqrt(np.einsum("...ij,...ij->...i",
                                                                        np.einsum("...i,...ij->...j",
                                                                                  tetra_triangle_edge_vectors,
                                                                                  anisotropy_invert_tensors.values(
                                                                                      tetra_triangles)),
                                                                        tetra_triangle_edge_vectors))

                        tetra_triangle_adjacent_edges = tetra_triangles[:, triangle_adjacent_edge_list]
                        tetra_triangle_adjacent_edge_vectors = positions.values(
                            tetra_triangle_adjacent_edges[..., 1]) - positions.values(
                            tetra_triangle_adjacent_edges[..., 0])

                        tetra_triangle_combined_vectors = tetra_triangle_adjacent_edge_vectors.sum(axis=2)
                        tetra_triangle_combined_lengths = np.sqrt(np.einsum("...ij,...ij->...i",
                                                                            np.einsum("...i,...ij->...j",
                                                                                      tetra_triangle_combined_vectors,
                                                                                      anisotropy_invert_tensors.values(
                                                                                          tetra_triangles)),
                                                                            tetra_triangle_combined_vectors))
                        ...
                        tetra_triangle_areas = np.sqrt((tetra_triangle_perimeters / 2.0) * (
                        tetra_triangle_perimeters / 2.0 - tetra_triangle_edge_lengths[..., 0]) * (
                                                       tetra_triangle_perimeters / 2.0 - tetra_triangle_edge_lengths[
                                                           ..., 1]) * (
                                                       tetra_triangle_perimeters / 2.0 - tetra_triangle_edge_lengths[
                                                           ..., 2]))
                        ...
                        vertex_positions[pid] = (positions.values(t) * tetra_triangle_areas[:, np.newaxis]).sum(
                            axis=0) / tetra_area
                    else:
                        cayley_menger_edges = np.array([[(p1, p2) for p2 in t] for p1 in t])
                        cayley_menger_edge_lengths = np.linalg.norm(
                            positions.values(cayley_menger_edges[..., 1]) - positions.values(
                                cayley_menger_edges[..., 0]), axis=2)
                        ...
                        if center_weights.min() > 0:
                            ...
                            opposite_triangle_sinuses = \
                            triangle_geometric_features(np.array([opposite_triangle]), positions, features=['sinus'])[
                                0][0]
                            opposite_triangle_cosinuses = \
                            triangle_geometric_features(np.array([opposite_triangle]), positions, features=['cosinus'])[
                                0][0]

                            opposite_triangle_sincos = (opposite_triangle_sinuses * opposite_triangle_cosinuses) / (
                            opposite_triangle_sinuses * opposite_triangle_cosinuses).sum()
                            if opposite_triangle_sincos.min() > 0:
                                vertex_positions[pid] = (
                                positions.values(t[1:]) * opposite_triangle_sincos[:, np.newaxis]).sum(axis=0)
                            else:
                ...
                if voronoi:
                    try:
                        ...
                        if anisotropic:
                            triangle_edge_vectors = positions.values(triangle_edges[..., 1]) - positions.values(
                                triangle_edges[..., 0])
                            triangle_edge_lengths = np.array([np.sqrt(np.einsum("...ij,...ij->...i",
                                                                                np.einsum("...i,...ij->...j",
                                                                                          triangle_edge_vectors,
                                                                                          anisotropy_invert_tensors.values(
                                                                                              [t[k], t[k], t[k]])),
                                                                                triangle_edge_vectors)) for k in
                                                              xrange(1, 4)])

                            triangle_adjacent_edges = t[1:][triangle_adjacent_edge_list]
                            triangle_adjacent_edge_vectors = positions.values(
                                triangle_adjacent_edges[..., 1]) - positions.values(triangle_adjacent_edges[..., 0])

                            triangle_combined_vectors = triangle_adjacent_edge_vectors.sum(axis=1)
                            triangle_combined_lengths = np.sqrt(np.einsum("...ij,...ij->...i",
                                                                          np.einsum("...i,...ij->...j",
                                                                                    triangle_combined_vectors,
                                                                                    anisotropy_invert_tensors.values(
                                                                                        t[1:])),
                                                                          triangle_combined_vectors))
                            ...
                            triangle_edge_lengths = np.linalg.norm(
                                positions.values(triangle_edges[..., 1]) - positions.values(triangle_edges[..., 0]),
                                axis=1)

                        triangle_perimeter = triangle_edge_lengths.sum()
                        vertex_positions[pid] = (positions.values(t[1:]) * triangle_edge_lengths[:, np.newaxis]).sum(
                            axis=0) / triangle_perimeter

                        triangle_sinuses = \
                        triangle_geometric_features(np.array([t[1:]]), positions, features=['sinus'])[0][0]
                        triangle_cosinuses = \
                        triangle_geometric_features(np.array([t[1:]]), positions, features=['cosinus'])[0][0]
                        ...
                        triangle_tangent_sums = triangle_tangents[..., triangle_edge_list].sum(axis=1) / \
                                                triangle_tangents[..., triangle_edge_list].sum()
                        # # print triangle_tangent_sums, np.maximum(triangle_tangent_sums,0)/np.maximum(triangle_tangent_sums,0).sum()
                        triangle_tangent_sums = np.maximum(triangle_tangent_sums, -1) / np.maximum(
                            triangle_tangent_sums, -1).sum()

                        triangle_sincos = (triangle_sinuses * triangle_cosinuses) / (
                        triangle_sinuses * triangle_cosinuses).sum()
                        ...
                        if surface:
                            ...
                            triangle_matching = []
                            for tetra_p in xrange(4):
                                triangle_matching.append(np.array(list(vq(
                                    np.concatenate([image_tetrahedra[:, :tetra_p], image_tetrahedra[:, tetra_p + 1:]],
                                                   axis=1), np.array([vertex_triangle])))))
                            vertex_other_tetra = image_tetrahedra[np.where(np.array(triangle_matching)[:, 1] == 0)[1]]
                            if len(vertex_other_tetra) > 0:
                                vertex_other_tetra = \
                                [other_tetra for other_tetra in vertex_other_tetra if not 1 in other_tetra][0]
                                vertex_other_tetra_point = [p for p in vertex_other_tetra if not p in t][0]
                            vertex_triangle_barycenter = positions.values(t[1:]).mean(axis=0)
                            ...
                            surface_triangle_intersection = intersecting_triangle(np.array(
                                [vertex_positions[pid], vertex_positions[pid] + 1000. * vertex_triangle_normal]),
                                                                                  surface_triangle_points)[0]
                            ...
                            if surface_triangle_intersection.sum() > 0:
                                ...
                                vertex_positions[pid] += 0.5 * (
                                    1 - np.tanh(2.0 * (surface_distance - 5.0))) * (
                                                             surface_distance * vertex_triangle_normal)
            ...
            if voronoi:
                ...
                if anisotropic:
                    ...
                    tetra_triangle_edge_vectors = positions.values(tetra_triangle_edges[..., 1]) - positions.values(
                        tetra_triangle_edges[..., 0])
                    tetra_triangle_edge_lengths = np.sqrt(np.einsum("...ij,...ij->...i", np.einsum("...i,...ij->...j",
                                                                                                   tetra_triangle_edge_vectors,
                                                                                                   anisotropy_invert_tensors.values(
                                                                                                       tetra_triangles)),
                                                                    tetra_triangle_edge_vectors))
                    ...
                    tetra_triangle_adjacent_edges = tetra_triangles[:, triangle_adjacent_edge_list]
                    tetra_triangle_adjacent_edge_vectors = positions.values(
                        tetra_triangle_adjacent_edges[..., 1]) - positions.values(tetra_triangle_adjacent_edges[..., 0])
                    ...
                    try:
                        tetra_triangle_edge_lengths = np.linalg.norm(
                            positions.values(tetra_triangle_edges[..., 1]) - positions.values(
                                tetra_triangle_edges[..., 0]), axis=2)
    ...
    _, unique_triangles = np.unique(np.ascontiguousarray(image_triangles).view(
        np.dtype((np.void, image_triangles.dtype.itemsize * image_triangles.shape[1]))), return_index=True)
    ...
    tetrahedra_triangle_matching = np.array(np.concatenate([vq(image_tetrahedra[:, 1:], image_triangles),
                                                            vq(np.concatenate([image_tetrahedra[:, 0][:, np.newaxis],
                                                                               image_tetrahedra[:, 2:]], axis=1),
                                                               image_triangles),
                                                            vq(np.concatenate([image_tetrahedra[:, :2],
                                                                               image_tetrahedra[:, -1][:, np.newaxis]],
                                                                              axis=1), image_triangles),
                                                            vq(image_tetrahedra[:, :-1], image_triangles)], axis=1),
                                            int)

    for i, t in enumerate(image_triangles):
        edge_vertices = np.unique(
            (np.where((tetrahedra_triangle_matching[1] == 0) & (tetrahedra_triangle_matching[0] == i))[0]) %
            image_tetrahedra.shape[0])
    ...
    _, unique_edges = np.unique(
        np.ascontiguousarray(image_edges).view(np.dtype((np.void, image_edges.dtype.itemsize * image_edges.shape[1]))),
        return_index=True)
    image_edges = image_edges[unique_edges]

    triangle_edge_matching = np.array(
        np.concatenate([vq(image_triangles[np.array(list(image_topomesh.wisps(1))), 1:], image_edges),
                        vq(np.concatenate([image_triangles[np.array(list(image_topomesh.wisps(1))), :1],
                                           image_triangles[np.array(list(image_topomesh.wisps(1))), 2:]], axis=1),
                           image_edges),
    ...
    for i, t in enumerate(image_edges):
        ...
        facet_edges = np.array(list(image_topomesh.wisps(1)))[np.unique((np.where(
            (triangle_edge_matching[1] == 0) & (triangle_edge_matching[0] == i))[0]) % image_topomesh.nb_wisps(1))]
    ...
    edge_point_matching = np.array(
        np.concatenate([vq(image_edges[:, 0], image_points), vq(image_edges[:, 1], image_points)], axis=1), int)
    for i, t in enumerate(image_points):
        cell_facets = np.unique(
            (np.where((edge_point_matching[1] == 0) & (edge_point_matching[0] == i))[0]) % image_edges.shape[0])
...
def tetrahedra_dual_triangular_topomesh(triangulation_topomesh, image_cell_vertex=None,
                                        triangular='star_split_regular_flat', **kwargs):
    ...
    image_topomesh = tetrahedra_dual_topomesh(triangulation_tetrahedra, positions, voronoi=voronoi, exterior=exterior,
    ...
    if image_cell_vertex is not None and vertex_motion:
        ...
        mesh_vertices = np.sort([image_topomesh.wisp_property('cells', 0)[v] if (
        len(image_topomesh.wisp_property('cells', 0)[v]) == 4) else np.concatenate(
            [[1], image_topomesh.wisp_property('cells', 0)[v]]) for v in image_topomesh.wisps(0) if
                                 len(image_topomesh.wisp_property('cells', 0)[v]) in [3, 4]])
        mesh_cell_vertex = array_dict(
            [v for v in image_topomesh.wisps(0) if len(image_topomesh.wisp_property('cells', 0)[v]) in [3, 4]],
            keys=mesh_vertices).to_dict()
        ...
        initial_vertex_positions = array_dict(
            image_topomesh.wisp_property('barycenter', 0).values(list(image_topomesh.wisps(0))),
            list(image_topomesh.wisps(0)))
    ...
    if len(triangular) == 0 or triangular is None:
        ...
        from openalea.mesh.property_topomesh_optimization import property_topomesh_edge_split_optimization, \
        ...
        if 'star' in triangular:
            ...
            for fid in image_topomesh.wisps(2):
                if image_topomesh.nb_regions(2, fid) == 2:
                    ...
                    interface_normals[fid] = triangulation_topomesh.wisp_property('normal', 0)[
                        list(image_topomesh.regions(2, fid))[0]]
        ...
        #   image_triangular_topomesh = optimize_topomesh(image_triangular_topomesh,
        #   omega_forces=dict([('taubin_smoothing',0.65),('planarization',0.1),('epidermis_planarization',0.1)]),
        #   omega_regularization_max=0.0,gradient_derivatives=None,cell_vertex_motion=False,
        #   image_cell_vertex=image_cell_vertex,edge_flip=False,display=False,iterations=10,iterations_per_step=1)
        ...
        if 'split' in triangular:
            ...
            while (n_flips + n_splits > image_triangular_topomesh.nb_wisps(1) / 100.) and (iterations < max_iterations):
                n_splits = property_topomesh_edge_split_optimization(image_triangular_topomesh,
                                                                     maximal_length=maximal_length, iterations=1)
                n_flips = property_topomesh_edge_flip_optimization(image_triangular_topomesh,
                                                                   omega_energies=dict([('neighborhood', 0.65)]),
                                                                   simulated_annealing=False, iterations=3)
                image_triangular_topomesh = optimize_topomesh(image_triangular_topomesh,
                                                              omega_forces=dict([('taubin_smoothing', 0.33)]),
                                                              omega_regularization_max=0.0, gradient_derivatives=None,
                                                              image_voxelsize=voxelsize,
                                                              cell_vertex_motion=vertex_preserving,
                                                              image_cell_vertex=image_cell_vertex, edge_flip=False,
                                                              display=False, iterations=10 - iterations,
        ...
        if 'realistic' in triangular:
            image_triangular_topomesh = optimize_topomesh(image_triangular_topomesh,
                                                          ...
                                                          cell_vertex_motion=vertex_preserving,
                                                          image_cell_vertex=image_cell_vertex, edge_flip=False,
                                                          display=False, iterations=20, iterations_per_step=1)
        elif 'regular' in triangular:
            image_triangular_topomesh = optimize_topomesh(image_triangular_topomesh, omega_forces=dict(
                [('taubin_smoothing', 0.65), ('convexity', 0.02)]), omega_regularization_max=0.0,
                                                          gradient_derivatives=None, image_voxelsize=voxelsize,
                                                          cell_vertex_motion=False, image_cell_vertex=image_cell_vertex,
                                                          edge_flip=False, display=False, iterations=50,
                                                          iterations_per_step=1)

        if 'projected' in triangular and surface_topomesh is not None:
            compute_topomesh_property(image_triangular_topomesh, 'normal', 2,
                                      object_positions=triangulation_topomesh.wisp_property('barycenter', 0))
            ...
            projected_positions = {}
            surface_triangle_intersection = np.array([intersecting_triangle(
                np.array([vertex_positions[v] - vertex_normals[v], vertex_positions[v] + 1000. * vertex_normals[v]]),
                surface_triangle_points)[0] for v in image_triangular_topomesh.wisps(0)])
            ...
            for v, t in zip(projectable_vertices, projected_triangles):
                ...
                projected_positions[v] = vertex_positions[v] + 0.5 * (1 - np.tanh(2.0 * (surface_distance - 5.0))) * (
                surface_distance * vertex_normals[v])
            image_triangular_topomesh.update_wisp_property('barycenter', 0, array_dict(
                [projected_positions[v] if projected_positions.has_key(v) else vertex_positions[v] for v in
                 image_triangular_topomesh.wisps(0)], list(image_triangular_topomesh.wisps(0))))
            ...
            image_triangular_topomesh = optimize_topomesh(image_triangular_topomesh,
                                                          ...
                                                          cell_vertex_motion=vertex_preserving,
                                                          image_cell_vertex=image_cell_vertex, edge_flip=False,
                                                          display=False, iterations=10, iterations_per_step=1)

        if 'flat' in triangular:
            image_triangular_topomesh = optimize_topomesh(image_triangular_topomesh, omega_forces=dict(
                [('taubin_smoothing', 0.65), ('planarization', 0.1), ('epidermis_planarization', 0.1)]),
                                                          omega_regularization_max=0.0, gradient_derivatives=None,
                                                          cell_vertex_motion=vertex_preserving,
                                                          image_cell_vertex=image_cell_vertex, edge_flip=False,
                                                          display=False, iterations=10, iterations_per_step=1)
            ...
            image_triangular_topomesh = optimize_topomesh(image_triangular_topomesh, omega_forces=dict(
                [('taubin_smoothing', 0.65), ('laplacian', 0.5), ('epidermis_planarization', 0.1)]),
                                                          omega_regularization_max=0.0, gradient_derivatives=None,
                                                          cell_vertex_motion=True, image_cell_vertex=image_cell_vertex,
                                                          edge_flip=False, display=False, iterations=10,
                                                          iterations_per_step=1)
            image_triangular_topomesh = optimize_topomesh(image_triangular_topomesh,
                                                          omega_forces=dict([('taubin_smoothing', 0.65)]),
                                                          omega_regularization_max=0.0, gradient_derivatives=None,
                                                          ...
                                                          cell_vertex_motion=vertex_preserving,
                                                          image_cell_vertex=image_cell_vertex, edge_flip=False,
                                                          display=False, iterations=3, iterations_per_step=1)
...
def triangulated_interface_topomesh(image_topomesh, maximal_length, inner_interfaces=True):
    ...
    for interface in image_topomesh.wisps(2):
        ...
        interface_edges = image_topomesh.wisp_property('vertices', 1).values(
            image_topomesh.wisp_property('borders', 2)[interface])
        ...
        if (len(interface_vertices) > 2) and (inner_interfaces or (len(interface_cells) == 1)):
            ...
            interface_positions = array_dict(image_topomesh.wisp_property('barycenter', 0).values(interface_vertices),
                                             interface_vertices)

            interface_center = interface_positions.values().mean(axis=0)

            interface_extension = 1 * (
            image_topomesh.wisp_property('barycenter', 0).values(interface_vertices) - interface_center)
            interface_pids = []
            ...
            interface_projectors = -np.einsum('ij,ij->i', interface_vertices_vectors,
                                              interface_eigenvectors[0][np.newaxis, :])[:, np.newaxis] * \
                                   interface_eigenvectors[0][np.newaxis, :]

            interface_vertex_positions = array_dict(
                interface_positions.values(interface_vertices) + interface_projectors, interface_vertices)

            projected_vertex_positions = np.transpose(
                [np.einsum('ij,ij->i', interface_vertices_vectors, interface_eigenvectors[2][np.newaxis, :]),
                 np.einsum('ij,ij->i', interface_vertices_vectors, interface_eigenvectors[1][np.newaxis, :]),
                 np.zeros_like(interface_vertices)])
            ...
            interface_edges = image_topomesh.wisp_property('vertices', 1).values(
                image_topomesh.wisp_property('borders', 2)[interface])
            interface_edge_lengths = np.linalg.norm(
                interface_positions.values(interface_edges[:, 1]) - interface_positions.values(interface_edges[:, 0]),
                axis=1)
            ...
            for e, l in zip(interface_edges, interface_edge_lengths):
                ...
                while not end_reached:
                    ...
                    next_proj_p = prev_proj_p + maximal_length * (
                    projected_vertex_positions[e[1]] - projected_vertex_positions[e[0]]) / l
                    if np.linalg.norm(interface_positions[e[1]] - next_p) > maximal_length / 2. and np.linalg.norm(
                                    interface_positions[e[0]] - next_p) < l:
                        ...
                        interface_triangulation_edges.append(
                            np.sort([prev_index, vq(np.array([e[1]]), interface_vertices)[0][0]]))
                        end_reached = True
            ...
            for b in interface_projected_borders:
                intersecting_borders = intersecting_borders or (
                intersecting_segment(b, interface_projected_borders).sum() > 0)
            ...
            if intersecting_borders:
                ...
                while intersecting_borders and (c < len(interface_cells) - 1):
                    ...
                    sphere_x = np.array(
                        [sphere_z[2] - sphere_z[1], sphere_z[0] - sphere_z[2], sphere_z[1] - sphere_z[0]])
                    ...
                    phi = np.arccos(
                        np.array(np.dot(np.array(interface_points) - cell_center, sphere_axes[2]) / rho, np.float16))
                    theta = np.arccos(
                        np.array(np.dot(np.array(interface_points) - cell_center, sphere_axes[1]) / (rho * np.sin(phi)),
                                 np.float16))
                    theta = theta - np.pi / 2.
            ...
            interface_triangles = np.array(
                [np.concatenate([interface_vertex_positions.values(e), [interface_center]]) for e in interface_edges])
            ...
            points_distance = [vq(np.array([p]), np.concatenate([np.array(proj_points[:i]).reshape(i, 3),
                                                                 np.array(proj_points[i + 1:]).reshape(
                                                                     len(points) - i - 1, 3)]))[1][0] for i, p in
                               enumerate(proj_points)]
            ...
            for p, proj_p in zip(points, proj_points):
                ...
                if vertex_matching[1][0] > maximal_length / 2.:
                    ...
                    if spherical_triangulation:
                        ...
                        theta = np.arccos(
                            np.array(np.dot(p - cell_center, sphere_axes[1]) / (rho * np.sin(phi)), np.float16))
                        theta = theta - np.pi / 2.
                        phi = phi - np.pi / 2
            ...
            while len(points) > 0:
                ...
                for i in xrange(6):
                    new_p = p + maximal_length * (
                    np.cos(i * np.pi / 3.) * interface_eigenvectors[2] + np.sin(i * np.pi / 3.) *
                    interface_eigenvectors[1])
                    proj_new_p = proj_p + maximal_length * np.array(
                        [np.cos(i * np.pi / 3.), np.sin(i * np.pi / 3.), 0.])
                    ...
                    if inside_triangle(new_p, interface_triangles).sum() > 0:
                        if vertex_matching[1][0] > maximal_length / 2.:
                            ...
                            if spherical_triangulation:
                                ...
                                theta = np.arccos(
                                    np.array(np.dot(new_p - cell_center, sphere_axes[1]) / (rho * np.sin(phi)),
                                             np.float16))
                                theta = theta - np.pi / 2.
                                phi = phi - np.pi / 2
                                sphere_new_p = np.array([10. * theta, 10. * np.log(np.tan(np.pi / 4. + phi / 2.)), 0.])
                                spherical_points.append(sphere_new_p)
                            if (len(processed_points) == 0) or (
                                np.float16(vq(np.array([new_p]), np.array(processed_points))[1][0]) > 0.):
                                if (len(points) == 0) or (
                                    np.float16(vq(np.array([new_p]), np.array(points))[1][0]) > 0.):
                                    points.append(new_p)
                ...
                points_distance = [vq(np.array([proj_p]), np.concatenate([np.array(proj_points[:i]).reshape(i, 3),
                                                                          np.array(proj_points[i + 1:]).reshape(
                                                                              len(points) - i - 1, 3)]))[1][0] for
                                   i, proj_p in enumerate(proj_points)]
                points_order = np.argsort(-np.array(points_distance))
                points = list(np.array(points)[points_order])
                proj_points = list(np.array(proj_points)[points_order])
            ...
            interface_projected_triangulation = np.array(triangulation_points)[:, :2][
                np.array(interface_triangulation_edges)]
            ...
            suppressed_interface_triangulation_edges = np.array(interface_triangulation_edges)[
                interface_triangulation_edges_intersecting_borders]
            interface_triangulation_edges = np.delete(interface_triangulation_edges,
                                                      interface_triangulation_edges_intersecting_borders, 0)
            ...
            interface_delaunay_triangulation_edges = np.sort(
                np.concatenate(interface_triangulation_triangles[:, triangle_edge_list]))
            _, unique_triangulation_edges = np.unique(np.ascontiguousarray(interface_delaunay_triangulation_edges).view(
                np.dtype((np.void, interface_delaunay_triangulation_edges.dtype.itemsize *
                          interface_delaunay_triangulation_edges.shape[1]))), return_index=True)
            interface_delaunay_triangulation_edges = interface_delaunay_triangulation_edges[unique_triangulation_edges]

            interface_missing_edges = np.array(interface_border_edges)[
                np.where(vq(np.array(interface_border_edges), interface_delaunay_triangulation_edges)[1] > 0)]
            interface_triangulation_additional_edges = interface_delaunay_triangulation_edges[
                np.where(vq(interface_delaunay_triangulation_edges, np.array(interface_triangulation_edges))[1] > 0)]
            interface_triangulation_edges_to_flip = interface_triangulation_additional_edges[
                np.sum(interface_triangulation_additional_edges >= inside_vertex_index, axis=1) > 0]
            ...
            if len(interface_triangulation_edges_to_flip) > 0:
                ...
                for i, t in enumerate(interface_triangulation_triangles):
                    if edge_matching[1][i] == 0:
                        edge_triangles[edge_matching[0][i]].append(i)
                edge_matching = vq(
                    np.concatenate([interface_triangulation_triangles[:, :1], interface_triangulation_triangles[:, 2:]],
                                   axis=1), interface_triangulation_edges_to_flip)
                for i, t in enumerate(interface_triangulation_triangles):
                    if edge_matching[1][i] == 0:
                        edge_triangles[edge_matching[0][i]].append(i)
                ...
                for i, e in enumerate(interface_triangulation_edges_to_flip):
                    edge_quadrilateral = np.unique(
                        np.array([interface_triangulation_triangles[t] for t in edge_triangles[i]]))
                    edge_diagonal = edge_quadrilateral[
                        np.where((edge_quadrilateral != e[0]) & (edge_quadrilateral != e[1]))]

                    if len(edge_diagonal) == 2:
                        aligned_points = False
                        for j, t in enumerate(edge_triangles[i]):
                            aligned_points = aligned_points | (np.array(
                                np.cross(interface_points[e[j]] - interface_points[edge_diagonal[0]],
                                         interface_points[e[j]] - interface_points[edge_diagonal[1]]),
                                np.float16).sum() == 0.)

                        intersecting_diagonals = intersecting_segment(np.array(triangulation_points)[:, :2][e],
                                                                      np.array(triangulation_points)[:, :2][
                                                                          edge_diagonal])

                        if (not aligned_points) and (intersecting_diagonals) and (
                            (edge_diagonal > inside_vertex_index).sum() == 0) or (
                            vq(np.array([edge_diagonal]), np.array(interface_triangulation_edges))[1] == 0):
                            for j, t in enumerate(edge_triangles[i]):
                            ...
                            edge_triangles = dict(
                                [(i, []) for i, e in enumerate(interface_triangulation_edges_to_flip)])
                            edge_matching = vq(interface_triangulation_triangles[:, 1:],
                                               interface_triangulation_edges_to_flip)
                            for i, t in enumerate(interface_triangulation_triangles):
                                if edge_matching[1][i] == 0:
                                    edge_triangles[edge_matching[0][i]].append(i)
                            edge_matching = vq(interface_triangulation_triangles[:, :2],
                                               interface_triangulation_edges_to_flip)
                            for i, t in enumerate(interface_triangulation_triangles):
                                if edge_matching[1][i] == 0:
                                    edge_triangles[edge_matching[0][i]].append(i)
                            edge_matching = vq(np.concatenate(
                                [interface_triangulation_triangles[:, :1], interface_triangulation_triangles[:, 2:]],
                                axis=1), interface_triangulation_edges_to_flip)
                            for i, t in enumerate(interface_triangulation_triangles):
                                if edge_matching[1][i] == 0:
                                    edge_triangles[edge_matching[0][i]].append(i)
            ...
            interface_triangulation_exterior_edges = interface_triangulation_additional_edges[
                np.sum(interface_triangulation_additional_edges > inside_vertex_index, axis=1) == 0]
            interface_triangles = np.array(
                [np.concatenate([np.array(triangulation_points)[e], [np.array([0, 0, 0])]]) for e in np.transpose(
                    [vq(interface_edges[:, 0], interface_pids)[0], vq(interface_edges[:, 1], interface_pids)[0]])])

            if len(interface_triangulation_exterior_edges) > 0:
                interface_triangulation_exterior_edges = interface_triangulation_exterior_edges[np.array([
                                                                                                             inside_triangle(
                                                                                                                 (
                                                                                                                 ...
                                                                                                                 triangulation_points[
                                                                                                                     e[
                                                                                                                         1]]) / 2.,
                                                                                                                 interface_triangles).sum() == 0
                                                                                                             for e in
                                                                                                             interface_triangulation_exterior_edges])]
            ...
            if len(interface_triangulation_exterior_edges) > 0:
                ...
                edge_matching = vq(
                    np.concatenate([interface_triangulation_triangles[:, :1], interface_triangulation_triangles[:, 2:]],
                                   axis=1), interface_triangulation_exterior_edges)
                for i, t in enumerate(interface_triangulation_triangles):
                    if edge_matching[1][i] == 0:
                        edge_triangles[edge_matching[0][i]].append(i)
                edge_triangles = array_dict(edge_triangles)
            ...
            if len(interface_triangulation_triangles) > 0:
                interface_delaunay_triangulation_edges = np.sort(
                    np.concatenate(interface_triangulation_triangles[:, triangle_edge_list]))
                _, unique_triangulation_edges = np.unique(
                    np.ascontiguousarray(interface_delaunay_triangulation_edges).view(np.dtype((np.void,
                                                                                                interface_delaunay_triangulation_edges.dtype.itemsize *
                                                                                                interface_delaunay_triangulation_edges.shape[
                                                                                                    1]))),
                    return_index=True)
                interface_delaunay_triangulation_edges = interface_delaunay_triangulation_edges[
                    unique_triangulation_edges]

                interface_triangulation_border_edges = interface_delaunay_triangulation_edges[
                    np.sum(interface_delaunay_triangulation_edges >= inside_vertex_index, axis=1) == 0]
                interface_triangulation_additional_border_edges = interface_triangulation_border_edges[
                    np.where(vq(interface_triangulation_border_edges, np.array(interface_border_edges))[1] > 0)]

                if len(interface_triangulation_additional_border_edges) > 0:
                    edge_triangles = dict(
                        [(i, []) for i, e in enumerate(interface_triangulation_additional_border_edges)])
                    edge_matching = vq(interface_triangulation_triangles[:, 1:],
                                       interface_triangulation_additional_border_edges)
                    for i, t in enumerate(interface_triangulation_triangles):
                        if edge_matching[1][i] == 0:
                            edge_triangles[edge_matching[0][i]].append(i)
                    edge_matching = vq(interface_triangulation_triangles[:, :2],
                    ...
                    for i, t in enumerate(interface_triangulation_triangles):
                        if edge_matching[1][i] == 0:
                            edge_triangles[edge_matching[0][i]].append(i)
                    edge_matching = vq(np.concatenate(
                        [interface_triangulation_triangles[:, :1], interface_triangulation_triangles[:, 2:]], axis=1),
                                       interface_triangulation_additional_border_edges)
                    for i, t in enumerate(interface_triangulation_triangles):
                        if edge_matching[1][i] == 0:
                            edge_triangles[edge_matching[0][i]].append(i)
                    ...
                    for i, e in enumerate(interface_triangulation_additional_border_edges):
                        for t in edge_triangles[i]:
                            ...
                            triangle_vertex = edge_triangulation_triangle[np.where(
                                (edge_triangulation_triangle != e[0]) & (edge_triangulation_triangle != e[1]))][0]
                            aligned_points = np.array(
                                np.cross(interface_points[triangle_vertex] - interface_points[e[0]],
                                         interface_points[triangle_vertex] - interface_points[e[1]]),
                                np.float16).sum() == 0.
                            if aligned_points:
                ...
                for t in interface_pids_edges:
                    ...
                    for e in t:
                        edge_matching = vq(np.array([e]), np.array(triangle_edge_vertices.values()).reshape(
                            len(triangle_edge_vertices), 2))
                ...
                if interface % 100 == 0:
                    ...
                    print "  --> Interface ", interface, " / ", image_topomesh.nb_wisps(2), '     [', (
                    interface_end_time - interface_start_time), 's]'
...
def delaunay_interface_topomesh(image_topomesh, inner_interfaces=True):
    ...
    def project(points, plane_center, normal_vector):
        ...
        projectors = -np.einsum('ij,ij->i', vectors, normal_vector[np.newaxis, :])[:, np.newaxis] * normal_vector[
                                                                                                    np.newaxis, :]
