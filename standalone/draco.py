...
from openalea.mesh.property_topomesh_creation import vertex_topomesh, edge_topomesh, triangle_topomesh, \
    tetrahedra_topomesh
...
class DracoMesh(object):
    ...
    def __init__(self, image=None, image_file=None, image_cell_vertex_file=None, triangulation_file=None,
    ...
    def compute_image_adjacency(self):
        ...
        self.image_graph = graph_from_image(self.segmented_image,
                                            spatio_temporal_properties=['volume', 'barycenter', 'L1'],
                                            ignore_cells_at_stack_margins=False, property_as_real=True)
        self.image_labels = np.array(list(self.image_graph.vertices()))
        self.image_cell_volumes = array_dict([self.image_graph.vertex_property('volume')[v] for v in self.image_labels],
                                             self.image_labels)
        ...
        image_cell_vertex_tetrahedra = np.delete(image_cell_vertex_tetrahedra,
                                                 np.where(image_cell_vertex_tetrahedra[:, 0] == 1)[0], axis=0)
        ...
        truncated_image_graph = graph_from_image(truncated_image, spatio_temporal_properties=['barycenter', 'L1'],
                                                 background=1, ignore_cells_at_stack_margins=True,
            ...
            if c > 1 and self.cell_layer[c] == 1:
                for n in truncated_image_graph.neighbors(c):
                    if n > 1 and self.cell_layer[n] != 1:
                        self.cell_layer[n] = 2
        self.point_topomesh.update_wisp_property('layer', 0, self.cell_layer.values(list(self.point_topomesh.wisps(0))),
                                                 list(self.point_topomesh.wisps(0)))

        self.layer_edge_topomesh = {}

        if (self.cell_layer.values() == 1).sum() > 1:
            L1_edges = np.array(
                [[(c, n) for n in self.image_graph.neighbors(c) if n > 1 and self.cell_layer[n] == 1] for c in
                 self.cell_layer.keys() if self.cell_layer[c] == 1])
        ...
        if (self.cell_layer.values() == 2).sum() > 1:
            L2_edges = np.array(
                [[(c, n) for n in self.image_graph.neighbors(c) if n > 1 and self.cell_layer[n] == 2] for c in
                 self.cell_layer.keys() if self.cell_layer[c] == 2])
        ...
        if (self.cell_layer.values() == 1).sum() > 1 and (self.cell_layer.values() == 2).sum() > 0:
            L1_L2_edges = np.array(
                [[(c, n) for n in self.image_graph.neighbors(c) if n > 1 and self.cell_layer[n] in [1, 2]] for c in
                 self.cell_layer.keys() if self.cell_layer[c] in [1, 2]])
            L1_L2_edges = np.concatenate([e for e in L1_L2_edges if len(e) > 0])
            L1_L2_edges = L1_L2_edges[L1_L2_edges[:, 1] > L1_L2_edges[:, 0]]

            L1_L2_additional_edges = np.array([[(c, n) for n in np.unique(
                np.array(self.image_cell_vertex.keys())[np.where(np.array(self.image_cell_vertex.keys()) == c)[0]]) if
                                                n > 1 and n != c and (n not in self.image_graph.neighbors(c)) and (
                                                self.cell_layer[n] in [1, 2])] for c in self.cell_layer.keys() if
                                               self.cell_layer[c] in [1, 2]])
            if len([e for e in L1_L2_additional_edges if len(e) > 0]) > 0:
                ...
                L1_L2_additional_edges = L1_L2_additional_edges[
                    L1_L2_additional_edges[:, 1] > L1_L2_additional_edges[:, 0]]
                L1_L2_edges = np.concatenate([L1_L2_edges, L1_L2_additional_edges])

            self.layer_triangle_topomesh['L1_L2'] = triangle_topomesh(triangles_from_adjacency_edges(L1_L2_edges),
                                                                      self.positions)
    ...
    def delaunay_adjacency_complex(self, surface_cleaning_criteria=['surface', 'exterior', 'distance', 'sliver']):
        ...
        self.delaunay_topomesh = delaunay_tetrahedrization_topomesh(self.positions,
                                                                    image_cell_vertex=self.image_cell_vertex,
                                                                    segmented_image=self.segmented_image,
                                                                    clean_surface=clean_surface,
                                                                    surface_cleaning_criteria=surface_cleaning_criteria)
        clean_tetrahedrization(self.delaunay_topomesh, clean_vertices=False)
        discarded_cells = np.array(list(self.delaunay_topomesh.wisps(0)))[np.where(np.array(
            map(len, [list(self.delaunay_topomesh.regions(0, v, 2)) for v in self.delaunay_topomesh.wisps(0)])) == 0)[
            0]]
    ...
    def adjacency_complex_optimization(self, n_iterations=1,
                                       omega_energies={'image': 10.0, 'geometry': 0.1, 'adjacency': 0.01}):
        ...
        self.optimized_delaunay_topomesh = tetrahedrization_topomesh_topological_optimization(
            self.optimized_delaunay_topomesh, image_cell_vertex=self.image_cell_vertex, omega_energies=omega_energies,
            image_graph=self.image_graph, n_iterations=n_iterations)
    ...
    def layer_adjacency_complex(self, layer_name='L1',
        ...
        Returns:
            None
        """
        layer_triangulation_topomesh = layer_triangle_topomesh_construction(self.layer_edge_topomesh[layer_name],
                                                                            self.positions,
                                                                            omega_criteria=omega_criteria,
                                                                            wall_surfaces=self.image_wall_surfaces,
                                                                            cell_volumes=self.image_cell_volumes)
    ...
    def construct_adjacency_complex(self, omega_criteria={'distance': 1.0, 'wall_surface': 2.0, 'clique': 10.0}):
        ...
        constructed_triangulation_topomesh = layered_tetrahedra_topomesh_construction(
            self.layer_triangle_topomesh['L1_L2'], self.positions, self.cell_layer, omega_criteria=omega_criteria,
            wall_surfaces=self.image_wall_surfaces, cell_volumes=self.image_cell_volumes)
    ...
    def mesh_image_surface(self, layers=[], voxelsize=8):
        ...
        if len(layers) == 0:
            binary_img[self.size[0] / 2:3 * self.size[0] / 2, self.size[1] / 2:3 * self.size[1] / 2,
            self.size[2] / 2:3 * self.size[2] / 2][self.segmented_image > 1] = 1
        else:
            layer_img = deepcopy(self.segmented_image)
            layer_img[
                (np.any(np.array([self.cell_layer.values(self.segmented_image) == l for l in layers]), axis=0)) | (
                self.segmented_image == 1)] = 1
            binary_img[self.size[0] / 2:3 * self.size[0] / 2, self.size[1] / 2:3 * self.size[1] / 2,
            self.size[2] / 2:3 * self.size[2] / 2][layer_img > 1] = 1

        binary_img = binary_img[0:binary_img.shape[0]:grid_voxelsize[0], 0:binary_img.shape[1]:grid_voxelsize[1],
                     0:binary_img.shape[2]:grid_voxelsize[2]]
        ...
        self.surface_topomesh.update_wisp_property('barycenter', 0, self.surface_topomesh.wisp_property('barycenter',
                                                                                                        0).values() + np.array(
            binary_img.shape) * self.voxelsize * grid_voxelsize / 4.)

        return self.surface_topomesh

    def dual_reconstruction(self, reconstruction_triangulation=None, adjacency_complex_degree=3,
                            cell_vertex_constraint=True, maximal_edge_length=None):
        ...
        self.dual_reconstruction_topomesh = tetrahedra_dual_triangular_topomesh(self.triangulation_topomesh,
                                                                                triangular=self.reconstruction_triangulation,
                                                                                image_cell_vertex=self.image_cell_vertex,
                                                                                voronoi=True,
                                                                                vertex_motion=cell_vertex_constraint,
                                                                                surface_topomesh=self.surface_topomesh,
                                                                                maximal_length=maximal_edge_length)
...
def draco_delaunay_adjacency_complex(input_draco,
                                     surface_cleaning_criteria=['surface', 'exterior', 'distance', 'sliver']):
    draco = deepcopy(input_draco)
    draco.delaunay_adjacency_complex(surface_cleaning_criteria)
    return draco


def draco_layer_adjacency_complex(input_draco, layer_name='L1',
                                  omega_criteria={'distance': 1.0, 'wall_surface': 2.0, 'clique': 10.0}):
    draco = deepcopy(input_draco)
...
def draco_construct_adjacency_complex(input_draco,
                                      omega_criteria={'distance': 1.0, 'wall_surface': 2.0, 'clique': 10.0}):
    draco = deepcopy(input_draco)
...
def draco_adjacency_complex_optimization(input_draco, n_iterations=1,
                                         omega_energies={'image': 10.0, 'geometry': 0.1, 'adjacency': 0.01}):
    draco = deepcopy(input_draco)
...
def draco_dual_reconstruction(input_draco, reconstruction_triangulation=None, adjacency_complex_degree=3,
                              cell_vertex_constraint=True, maximal_edge_length=None):
    draco = deepcopy(input_draco)
    draco.dual_reconstruction(reconstruction_triangulation, adjacency_complex_degree, cell_vertex_constraint,
                              maximal_edge_length)
    return draco
