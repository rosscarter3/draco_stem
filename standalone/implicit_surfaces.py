...
def implicit_surface(density_field, size, resolution, iso=0.5):
    ...
    surface_points = (np.array(surface_points)) * (
        size * resolution / np.array(density_field.shape)) - size * resolution / 2.
...
def marching_cubes(field, iso=0.5):
    from skimage.measure import marching_cubes
    surface_points, surface_triangles = marching_cubes(field, iso)
    ...
    #     surface_points = (np.array(poly_data_object.points) - np.array(
    #         [abs(grid_points / 2.), abs(grid_points / 2.), abs(grid_points / 2.)])[np.newaxis, :]) * (
    #                      grid_max / abs(grid_points / 2.))
...
#
#     polydata_points = np.array(
#         [field_polydata.GetPoints().GetPoint(p) for p in xrange(field_polydata.GetPoints().GetNumberOfPoints())])
#     polydata_triangles = np.array([[field_polydata.GetCell(t).GetPointIds().GetId(i) for i in xrange(3)] for t in
#                                    xrange(field_polydata.GetNumberOfCells())])
#
...
def implicit_surface_topomesh(density_field, size, resolution, iso=0.5, center=True):
    ...
    _, unique_edges = np.unique(np.ascontiguousarray(surface_edges).view(
        np.dtype((np.void, surface_edges.dtype.itemsize * surface_edges.shape[1]))), return_index=True)
    ...
    surface_topomesh.update_wisp_property('barycenter', 0, array_dict(surface_points),
                                          keys=list(surface_topomesh.wisps(0)))
...
def spherical_density_function(positions, sphere_radius, k=0.1):
    ...
    def density_func(x, y, z):
        ...
        for p in positions.keys():
            cell_distances = np.power(
                np.power(x - positions[p][0], 2) + np.power(y - positions[p][1], 2) + np.power(z - positions[p][2], 2),
                0.5)
...
def point_spherical_density(positions, points, sphere_radius=5, k=1.0):
    return spherical_density_function(positions, sphere_radius=sphere_radius, k=k)(points[:, 0], points[:, 1],
                                                                                   points[:, 2])
