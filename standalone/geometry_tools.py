...
def tetra_geometric_features(tetrahedra, positions, features=['volume', 'max_distance']):
    ...
    tetra_edge_lengths = np.linalg.norm(positions.values(tetra_edges[..., 1]) - positions.values(tetra_edges[..., 0]),
                                        axis=3)

    tetra_sorted_edge_lengths = np.sort(np.linalg.norm(
        positions.values(tetrahedra[:, tetra_edge_list][..., 1]) - positions.values(
            tetrahedra[:, tetra_edge_list][..., 0]), axis=2))
    ...
    if 'volume' in features or 'eccentricity' in features:
        tetra_volumes = np.abs(np.sum(
            (tetra_positions[:, 0] - tetra_positions[:, 3]) * np.cross(tetra_positions[:, 1] - tetra_positions[:, 3],
                                                                       tetra_positions[:, 2] - tetra_positions[:, 3]),
            axis=1)) / 6.0
    ...
    if 'area' in features or 'eccentricity' in features:
        ...
        tetra_triangle_areas = np.sqrt(
            (tetra_triangle_perimeters / 2.0) * (tetra_triangle_perimeters / 2.0 - tetra_edge_lengths[..., 0]) * (
            tetra_triangle_perimeters / 2.0 - tetra_edge_lengths[..., 1]) * (
            tetra_triangle_perimeters / 2.0 - tetra_edge_lengths[..., 2]))
        tetra_areas = np.sum(tetra_triangle_areas, axis=1)

        tetra_triangle_eccentricities = 1. - (12.0 * np.sqrt(3) * tetra_triangle_areas) / np.power(
            tetra_triangle_perimeters, 2.0)
        ...
        tetra_triangle_sinuses[..., 0] = np.sqrt(np.array(1.0 - np.power(
            tetra_edge_lengths[..., 1] ** 2 + tetra_edge_lengths[..., 2] ** 2 - tetra_edge_lengths[..., 0] ** 2,
            2.0) / np.power(2.0 * tetra_edge_lengths[..., 1] * tetra_edge_lengths[..., 2], 2.0), np.float16))
        tetra_triangle_sinuses[..., 1] = np.sqrt(np.array(1.0 - np.power(
            tetra_edge_lengths[..., 2] ** 2 + tetra_edge_lengths[..., 0] ** 2 - tetra_edge_lengths[..., 1] ** 2,
            2.0) / np.power(2.0 * tetra_edge_lengths[..., 2] * tetra_edge_lengths[..., 0], 2.0), np.float16))
        tetra_triangle_sinuses[..., 2] = np.sqrt(np.array(1.0 - np.power(
            tetra_edge_lengths[..., 0] ** 2 + tetra_edge_lengths[..., 1] ** 2 - tetra_edge_lengths[..., 2] ** 2,
            2.0) / np.power(2.0 * tetra_edge_lengths[..., 0] * tetra_edge_lengths[..., 1], 2.0), np.float16))
        ...
        tetra_triangle_sinus_eccentricities = 1.0 - (2.0 * (
        tetra_triangle_sinuses[..., 0] + tetra_triangle_sinuses[..., 1] + tetra_triangle_sinuses[..., 2])) / (
                                                    3 * np.sqrt(3))
        ...
        tetra_sorted_edge_lengths = np.sort(
            np.linalg.norm(positions.values(tetra_edges[..., 1]) - positions.values(tetra_edges[..., 0]), axis=2))
    ...
    if 'circumscribed_sphere_center' in features or 'circumscribed_sphere_radius' in features or 'radius_edge_ratio' in features:
        ...
        for t in tetrahedra:
            ...
            tetra_triangle_edge_lengths = np.linalg.norm(
                positions.values(tetra_triangle_edges[..., 1]) - positions.values(tetra_triangle_edges[..., 0]), axis=2)

            try:
                cayley_menger_edges = np.array([[(p1, p2) for p2 in t] for p1 in t])
                cayley_menger_edge_lengths = np.linalg.norm(
                    positions.values(cayley_menger_edges[..., 1]) - positions.values(cayley_menger_edges[..., 0]),
                    axis=2)
    ...
    if 'max_dihedral_angle' in features or 'min_dihedral_angle' in features:
        ...
        tetra_triangle_normals = tetra_triangle_normals / np.linalg.norm(tetra_triangle_normals, axis=2)[:, :,
                                                          np.newaxis]
        ...
        normal_orientation = np.sign(
            np.einsum('...ij,...ij->...i', tetra_triangle_normals, tetra_triangle_tetra_vector))
        ...
        tetra_dihedral_cosines = np.einsum('...ij,...ij->...i', tetra_dihedral_normals[:, :, 0],
                                           tetra_dihedral_normals[:, :, 1])
...
def triangle_geometric_features(triangles, positions, features=['area', 'max_distance']):
    ...
    triangle_edge_lengths = np.linalg.norm(
        positions.values(triangle_edges[..., 1]) - positions.values(triangle_edges[..., 0]), axis=2)
    ...
    triangle_features['area'] = np.sqrt((triangle_features['perimeter'] / 2.0) * (
    triangle_features['perimeter'] / 2.0 - triangle_edge_lengths[..., 0]) * (
                                        triangle_features['perimeter'] / 2.0 - triangle_edge_lengths[..., 1]) * (
                                        triangle_features['perimeter'] / 2.0 - triangle_edge_lengths[..., 2]))

    triangle_features['eccentricity'] = 1. - (12.0 * np.sqrt(3) * triangle_features['area']) / np.power(
        triangle_features['perimeter'], 2.0)
    ...
    triangle_features['sinus'][:, 0] = np.sqrt(np.array(1.0 - np.power(
        triangle_edge_lengths[..., 1] ** 2 + triangle_edge_lengths[..., 2] ** 2 - triangle_edge_lengths[:, 0] ** 2,
        2.0) / np.power(2.0 * triangle_edge_lengths[..., 1] * triangle_edge_lengths[..., 2], 2.0), np.float16))
    triangle_features['sinus'][:, 1] = np.sqrt(np.array(1.0 - np.power(
        triangle_edge_lengths[..., 2] ** 2 + triangle_edge_lengths[..., 0] ** 2 - triangle_edge_lengths[:, 1] ** 2,
        2.0) / np.power(2.0 * triangle_edge_lengths[..., 2] * triangle_edge_lengths[..., 0], 2.0), np.float16))
    triangle_features['sinus'][:, 2] = np.sqrt(np.array(1.0 - np.power(
        triangle_edge_lengths[..., 0] ** 2 + triangle_edge_lengths[..., 1] ** 2 - triangle_edge_lengths[:, 2] ** 2,
        2.0) / np.power(2.0 * triangle_edge_lengths[..., 0] * triangle_edge_lengths[..., 1], 2.0), np.float16))
    ...
    triangle_features['cosinus'][:, 0] = (triangle_edge_lengths[..., 1] ** 2 + triangle_edge_lengths[
        ..., 2] ** 2 - triangle_edge_lengths[:, 0] ** 2) / (
                                         2.0 * triangle_edge_lengths[..., 1] * triangle_edge_lengths[..., 2])
    triangle_features['cosinus'][:, 1] = (triangle_edge_lengths[..., 2] ** 2 + triangle_edge_lengths[
        ..., 0] ** 2 - triangle_edge_lengths[:, 1] ** 2) / (
                                         2.0 * triangle_edge_lengths[..., 2] * triangle_edge_lengths[..., 0])
    triangle_features['cosinus'][:, 2] = (triangle_edge_lengths[..., 0] ** 2 + triangle_edge_lengths[
    ...
    triangle_features['orthocenter'] = (positions.values(triangles) * triangle_features['tangent'][:, :,
                                                                      np.newaxis]).sum(axis=1) / (triangle_features[
                                                                                                      'tangent'].sum(
        axis=1) + 1e-5)[:, np.newaxis]
    ...
    triangle_features['circumscribed_circle_center'] = (positions.values(triangles) * triangle_sincos[:, :,
    ...
    triangle_features['projected_circumscribed_circle_center'] = np.copy(
        triangle_features['circumscribed_circle_center'])
    if triangle_sincos.min() < 0:
        ...
        triangle_features['projected_circumscribed_circle_center'][np.where(triangle_sincos < 0)[0]] = positions.values(
            projected_triangle_edges[projected_triangle_sincos < 0]).mean(axis=1)
